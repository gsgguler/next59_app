...
// Fetch /fixtures/statistics for a given league+season pilot.
// Stores raw responses only — normalization runs via af_normalize_fixture_statistics() SQL fn.
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Default: PL 2023-2024 pilot
    const leagueAfId: number = body.league_id ?? 39;
    const seasonYear: number = body.season ?? 2023;
    // chunk_offset + chunk_size allow pagination to avoid 150s timeout
    ...
    for (const m of batch) {
      ...
      try {
        ...
        await supabase.from("api_football_fixture_statistics_raw").insert({
          match_id: m.id,
          api_football_fixture_id: fixtureId,
          endpoint,
          response_hash: null,
