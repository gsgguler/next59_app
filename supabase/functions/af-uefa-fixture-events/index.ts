...
// Fetch /fixtures/events for UEFA club competition fixtures.
// Source: af_uefa_fixtures (not matches table).
// Stores raw only — normalization runs via af_normalize_uefa_fixture_events() SQL fn.
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const f of batch) {
      ...
      const hash = `uefa-events-${fixtureId}`;

      const { data: existing } = await supabase
        .from("af_uefa_fixture_events_raw")
        .select("id")
      ...
      try {
        ...
        await supabase.from("af_uefa_fixture_events_raw").insert({
          af_uefa_fixture_id: f.id,
          api_football_fixture_id: fixtureId,
          af_league_id: f.af_league_id,
          af_season: f.af_season,
