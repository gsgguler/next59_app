...
/*
  # Feature Matrix V2 — Fix stats pass rn column reference

  ## Summary
  PASS 2 of ml_populate_feature_matrix_v2_competition referenced fl.rn from
  _fm_l5_form, but that temp table does not expose rn (only the aggregated
  form columns). Fix: join team_elo_snapshots for match_date ordering in
  team_stats, derive srn from match_date/match_id directly without needing rn.
*/
...
BEGIN
  ...
  -- Uses team_elo_snapshots (not _fm_l5_form) for match ordering to avoid rn dependency
  ...
  WITH team_stats AS (
    ...
    JOIN model_lab.team_elo_snapshots t
      ON (t.match_id=ms.match_id AND (t.home_team_id=ms.team_id OR t.away_team_id=ms.team_id))
    WHERE t.elo_version=p_elo_version
      AND t.competition_name=p_competition_name
      AND ms.half='FT'
      AND ms.total_shots IS NOT NULL
  ),
  ranked_s AS (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY team_id ORDER BY match_date, match_id) AS srn
    FROM team_stats
  ),
  lagged_s AS (
    SELECT match_id, team_id,
