/*
  # generate_prematch_brain_package — Sub-Brain + Master Brain Orchestration

  ## Purpose
  Runs all 7 allowed sub-brains against a single match, then aggregates into
  a Master Brain verdict. Writes results to the three brain tables.

  ## Allowed Sub-Brains
  1. probability    — p_home / p_draw / p_away + confidence
  2. draw_risk      — draw risk level + pressure explanation + league prior + calibration gap
  3. upset_risk     — favorite fragility + upset probability + overconfidence warning
  4. tempo          — expected tempo (low/balanced/high) from goals/form/stats only
  5. late_pressure  — late goal pressure (low/medium/high) from closeness + draw risk
  6. calibration    — league home/draw/away bias + production_candidate status
  7. data_quality   — feature quality tier + ELO/lineup readiness + warning severity

  ## Explicitly Disabled
  - player stamina, manager substitution, minute-by-minute event, card prediction, exact score

  ## Data Sources (read-only, all in model_lab schema)
  - model_lab.prematch_prediction_drafts
  - model_lab.upcoming_match_readiness
  - model_lab.league_calibration_state
  - model_lab.match_feature_matrix_v2
  - model_lab.league_draw_priors
  - public.team_elo_snapshots / public.team_elo_ratings (fallback)
*/
...
BEGIN
  ...
  -- ─────────────────────────────────────────────────────────
  SELECT c.name, ht.name, at2.name
  INTO v_competition, v_home_name, v_away_name
  ...
  FROM public.team_elo_snapshots tes
  ...
  IF v_elo_home IS NULL THEN
    ...
    FROM public.matches m
    JOIN public.team_elo_ratings ter ON ter.team_id = m.home_team_id
    WHERE m.id = p_match_id
  ...
  IF v_elo_away IS NULL THEN
    ...
    FROM public.matches m
    JOIN public.team_elo_ratings ter ON ter.team_id = m.away_team_id
    WHERE m.id = p_match_id
    ORDER BY ter.last_match_date DESC NULLS LAST
    LIMIT 1;
  ...
  v_p_home    := COALESCE((v_draft.probabilities->>'p_home')::numeric, 0.40);
  v_p_draw    := COALESCE((v_draft.probabilities->>'p_draw')::numeric, 0.28);
  v_p_away    := COALESCE((v_draft.probabilities->>'p_away')::numeric, 0.32);
...
GRANT EXECUTE ON FUNCTION model_lab.generate_prematch_brain_package(uuid, text) TO service_role;

-- ──────────────────────────────────────────────
-- Public wrappers
-- ──────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.ml_generate_prematch_brain_package(
  p_match_id     uuid,
  p_triggered_by text DEFAULT NULL
)
RETURNS jsonb
...
AS $$
BEGIN
  RETURN model_lab.generate_prematch_brain_package(p_match_id, p_triggered_by);
END;
$$;

GRANT EXECUTE ON FUNCTION public.ml_generate_prematch_brain_package(uuid, text) TO authenticated;

-- Helper: get latest brain package for a match (for admin UI)
CREATE OR REPLACE FUNCTION public.ml_get_latest_brain_package(p_match_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT jsonb_build_object(
    'brain_run_id',  pbr.id,
    'status',        pbr.status,
    'generated_at',  pbr.generated_at,
    'master_brain', jsonb_build_object(
      'final_readiness',        pmbo.final_readiness,
      'final_confidence',       pmbo.final_confidence,
      'scenario_tone',          pmbo.scenario_tone,
      'publish_recommendation', pmbo.publish_recommendation,
      'master_summary',         pmbo.master_summary,
      'warnings',               pmbo.warnings_json
    ),
    'sub_brains', (
      SELECT jsonb_object_agg(pbo.brain_name, jsonb_build_object(
        'output',           pbo.output_json,
        'confidence_score', pbo.confidence_score,
        'warning_level',    pbo.warning_level
      ))
      FROM model_lab.prematch_brain_outputs pbo
      WHERE pbo.brain_run_id = pbr.id
    )
  )
  FROM model_lab.prematch_brain_runs pbr
  JOIN model_lab.prematch_master_brain_outputs pmbo ON pmbo.brain_run_id = pbr.id
  WHERE pbr.match_id = p_match_id
    AND pbr.status = 'completed'
  ORDER BY pbr.generated_at DESC
  LIMIT 1;
$$;

GRANT EXECUTE ON FUNCTION public.ml_get_latest_brain_package(uuid) TO authenticated;
