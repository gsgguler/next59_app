/*
  # Fix assess_upcoming_match_readiness — ELO schema and column names

  ## Problem
  The function referenced `public.team_elo_ratings` (wrong schema) and used
  columns `rated_elo` / `rated_through` that don't exist. The correct table is
  `model_lab.team_elo_ratings` with columns `elo_overall` / `last_match_date`.
  The elo_version filter also did not apply (no such column).

  ## Changes
  - ELO lookup: `public.team_elo_ratings` → `model_lab.team_elo_ratings`
  - ELO column: `rated_elo` → `elo_overall`
  - Sort column: `rated_through` → `last_match_date`
  - Removed `elo_version` filter (column does not exist on this table)
...
-- 1. ELO readiness (model_lab.team_elo_ratings — elo_overall, last_match_date)
SELECT elo_overall, last_match_date INTO v_elo_home
FROM model_lab.team_elo_ratings
WHERE team_id = v_match.home_team_uuid
ORDER BY last_match_date DESC NULLS LAST LIMIT 1;

SELECT elo_overall, last_match_date INTO v_elo_away
FROM model_lab.team_elo_ratings
WHERE team_id = v_match.away_team_uuid
ORDER BY last_match_date DESC NULLS LAST LIMIT 1;

v_elo_home_val := (v_elo_home).elo_overall;
v_elo_away_val := (v_elo_away).elo_overall;
