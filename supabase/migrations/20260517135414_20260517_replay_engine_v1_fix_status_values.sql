/*
  # Fix replay function status values to match check constraint
  Status CHECK allows: 'pending', 'running', 'done', 'failed'
  Replace 'completed' -> 'done', 'completed_with_errors' -> 'failed'
*/
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- 4d. ELO win probability (Bradley-Terry logistic)
      ...
      v_draw_p := LEAST(c_base_draw + c_draw_sensitivity * v_elo_closeness, 0.38);
      ...
      IF v_actual_result IS NOT NULL THEN
        ...
        -- 4o. Calibration update (frozen during COVID)
        ...
        ELSE
          -- COVID era: append frozen-state event
          INSERT INTO model_lab.league_calibration_events (
            competition_name, model_version, elo_version,
            match_id, match_date, season_label, matches_evaluated,
            rolling_brier_l50, rolling_logloss_l50, rolling_accuracy_l50,
