/*
  # Fix generate_prematch_brain_package: format() specifiers

  PostgreSQL's format() function only supports %s, %I, %L — not C printf specifiers
  like %.1f. Replaced all format() calls using %.1f with string concatenation using
  round(..., 1)::text and to_char() for the master summary float formatting.
*/
...
-- ELO: prefer feature matrix, then team_elo_snapshots, then team_elo_ratings
...
IF v_p_home >= v_p_away THEN
  v_favorite_side := 'home'; v_favorite_p := v_p_home; v_underdog_p := v_p_away;
ELSE
  v_favorite_side := 'away'; v_favorite_p := v_p_away; v_underdog_p := v_p_home;
END IF;
...
IF v_has_stats AND v_combined_goals >= 3.5 THEN
  ...
  v_tempo_warn   := 'none'; v_tempo_conf := 0.70;
ELSIF v_combined_goals >= 2.5 THEN
  ...
  v_tempo_warn   := 'none'; v_tempo_conf := 0.55;
ELSE
  v_tempo_level := 'low';
...
IF v_closeness >= 0.70 AND v_p_draw >= 0.26 THEN
  ...
  v_late_warn   := 'low'; v_late_conf := 0.68;
...
IF v_cal_state.competition_name IS NULL THEN
  v_cal_warn := 'high'; v_cal_conf := 0.20;
  v_brier_status := 'no_calibration_data'; v_is_candidate := false;
ELSIF v_cal_brier <= 0.220 THEN
  v_brier_status := 'excellent'; v_cal_warn := 'none'; v_cal_conf := 0.85;
ELSIF v_cal_brier <= 0.240 THEN
  v_brier_status := 'good'; v_cal_warn := 'none'; v_cal_conf := 0.70;
ELSIF v_cal_brier <= 0.255 THEN
  v_brier_status := 'acceptable'; v_cal_warn := 'low'; v_cal_conf := 0.55;
ELSE
  v_brier_status := 'degraded'; v_cal_warn := 'high'; v_cal_conf := 0.30;
END IF;
