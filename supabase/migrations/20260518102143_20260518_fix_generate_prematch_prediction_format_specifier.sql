/*
  # Fix generate_prematch_prediction format() specifier bug

  ## Summary
  PostgreSQL's format() only supports %s, %I, %L specifiers — not %f.
  The function used format('brier %.3f > 0.26; ...', v_cal_brier) which
  crashes with "unrecognized format() type specifier '.'".

  Fix: use %s with explicit ::text cast and round() for display.
  Same bug exists in the warning for draw/upset/calibration — checked and
  only the brier line uses %.3f; all others use %s.
*/
...
IF v_cal_brier IS NOT NULL AND v_cal_brier > 0.26 THEN
  v_confidence := v_confidence - 0.05;
  -- FIX: use %s with round() cast — PostgreSQL format() does not support %f
  v_warnings := array_append(v_warnings,
