/*
  # Fix generate_prematch_brain_package: two record field issues

  ## Problems fixed
  1. `v_readiness record` → typed as `model_lab.upcoming_match_readiness` row type
     so `standings_readiness`, `injuries_readiness`, etc. are accessible directly.
  2. `v_cal.is_production_candidate` — column does NOT exist on `league_calibration_state`.
     Old `record` type silently failed at runtime. Replaced with NULL::boolean (not used by any consumer).

  ## Previous migration applied DROP FUNCTION already — this does the final correct version.
*/
...
v_result := jsonb_build_object(
  ...
  'calibration', jsonb_build_object(
    ...
    'production_candidate', NULL::boolean  -- column removed from league_calibration_state
  ),
