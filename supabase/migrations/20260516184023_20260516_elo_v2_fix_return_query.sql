...
/*
  # ELO V2 — Fix RETURN QUERY team count
  Uses home_team_id/away_team_id union to count distinct teams.
*/
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      UPDATE _elo_state_v2 SET current_elo = v_elo_home + v_delta_home
      WHERE team_id = v_match.home_team_id;

      UPDATE _elo_state_v2 SET current_elo = v_elo_away + v_delta_away
    ...
    RETURN QUERY
      SELECT
        ...
        v_match_count::integer,
        (SELECT COUNT(DISTINCT t.team_id)::integer
         FROM (
           SELECT home_team_id AS team_id FROM model_lab.team_elo_snapshots
           WHERE elo_version = p_elo_version AND competition_id = v_comp.competition_id
           UNION
           SELECT away_team_id FROM model_lab.team_elo_snapshots
           WHERE elo_version = p_elo_version AND competition_id = v_comp.competition_id
         ) t),
