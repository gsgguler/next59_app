/*
  # Fix assess_upcoming_match_readiness — use scalar variables for ELO

  ## Problem
  After SELECT INTO a generic record, accessing named fields with (v_elo_home).elo_overall
  fails because PL/pgSQL doesn't know the field layout at compile time.

  ## Fix
  Replace `v_elo_home record` / `v_elo_away record` with simple scalar numerics
  `v_elo_home_found boolean` + `v_elo_home_val numeric` fetched directly.
...
-- 1. ELO readiness — scalar SELECT into numeric variables
