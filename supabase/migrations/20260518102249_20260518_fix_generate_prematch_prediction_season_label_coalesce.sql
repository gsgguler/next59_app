/*
  # Fix generate_prematch_prediction season_label null constraint

  ## Summary
  The 2025/26 competition_seasons rows have football_data_uk_season_label = NULL
  because that column is only populated for historical CSV-sourced data.
  The INSERT into prematch_prediction_drafts has a NOT NULL constraint on season_label.
  
  Fix: COALESCE v_season_label with the seasons.label value (e.g. "2025-2026"),
  falling back to competition_season_id text if both are null.
*/
...
-- Get competition name + season label; fall back to seasons.label when fd_uk_season_label is null
SELECT c.name,
