/*
  # TASK 2 — Draw Formula V2: League-Specific Draw Prior + Reduced Sensitivity

  Creates model_lab.ml_replay_competition_season_v2() which differs from v1 in exactly
  three ways to isolate the draw recalibration effect:

  1. prediction_formula tag changed to 'formula_v2_draw_recalibrated'
  2. Draw heuristic replaced with league-specific prior from league_draw_priors table:
       closeness = 1 - abs(elo_gap / 400) * 2  (centred: 0 when evenly matched, 1 when gap=200)
       Wait — spec formula uses:
         closeness = 1 - abs(p_home_cal - 0.5) * 2
         p_draw_raw = league_draw_prior + 0.08 * (closeness - 0.5)
       After ELO win probability is computed, p_home_cal = v_elo_win_p
         closeness = 1 - abs(v_elo_win_p - 0.5) * 2
         p_draw_raw = league_prior + 0.08 * (closeness - 0.5)
         p_draw = clip(p_draw_raw, 0.10, 0.32)
  3. draw_bounds: min 0.10, max 0.32 (vs v1: unconstrained min, max 0.38)

  Everything else (ELO, form modifier, tier compression, calibration, COVID freeze) is
  identical to v1, ensuring a clean A/B comparison.

  Function signature:
    ml_replay_competition_season_v2(
      p_run_key          text,
      p_competition_name text,
      p_season_label     text
    ) RETURNS TABLE(run_id uuid, total_matches int, processed_matches int,
                    failed_matches int, status text)
*/
...
DECLARE
  ...
  -- V2 DRAW CONSTANTS (changed from v1)
  c_draw_sensitivity  constant numeric := 0.08;   -- was 0.12
  c_draw_min          constant numeric := 0.10;
  c_draw_max          constant numeric := 0.32;   -- was 0.38 with global floor 0.252
  -- unchanged
  ...
  -- V2: league draw prior (loaded once before loop)
...
BEGIN
  ...
  -- 2. Load league-specific draw prior (V2 only)
  ...
  -- Fallback: if no prior found, use conservative global estimate
  ...
  BEGIN
    ...
    -- 5d. ELO win probability (Bradley-Terry logistic) — identical to v1
    v_elo_win_p := 1.0 / (1.0 + power(10.0, -v_elo_gap / 400.0));

    -- 5e. V2 DRAW HEURISTIC (replaces v1 global base_draw + elo_closeness)
    --   closeness: 1 when teams evenly matched (elo_win_p=0.5), 0 when dominant
    --   formula from spec:
    --     closeness = 1 - abs(p_home_cal - 0.5) * 2
    --     p_draw_raw = league_draw_prior + 0.08 * (closeness - 0.5)
    --     p_draw = clip(p_draw_raw, 0.10, 0.32)
    v_closeness := 1.0 - ABS(v_elo_win_p - 0.5) * 2.0;  -- [0,1]
    ...
    -- 5h. Tier compression — identical to v1
    ...
    -- 5i. Calibration correction — identical to v1
    ...
    -- 5j. Renormalize — identical to v1
    ...
    IF v_actual_result IS NOT NULL THEN
      ...
      -- 5o. Calibration update (frozen during COVID) — identical to v1
      IF NOT v_is_covid THEN
        ...
        SELECT
          ...
          AVG(CASE WHEN e.actual_result='H' THEN 1.0 ELSE 0.0 END)
            - AVG(p2.p_home)                                               AS h_bias,
          AVG(CASE WHEN e.actual_result='D' THEN 1.0 ELSE 0.0 END)
            - AVG(p2.p_draw)                                               AS d_bias,
          AVG(CASE WHEN e.actual_result='A' THEN 1.0 ELSE 0.0 END)
            - AVG(p2.p_away)                                               AS a_bias,
...
-- Grant execute to authenticated (admin check inside via SECURITY DEFINER calling admin-only tables)
