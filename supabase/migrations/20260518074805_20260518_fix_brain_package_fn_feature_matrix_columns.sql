/*
  # Fix generate_prematch_brain_package: match_feature_matrix_v2 column names

  match_feature_matrix_v2 stores features as flat columns, not a jsonb "features" field.
  The correct column names for L5 goal stats are:
    - rolling_goals_for_home_l5  (not v_feat.features->>'home_l5_goals_scored')
    - rolling_goals_for_away_l5  (not v_feat.features->>'away_l5_goals_scored')

  Also updated: ELO can be read directly from the feature matrix (pre_match_elo_home/away),
  avoiding need to join team_elo_snapshots when feature matrix exists.
*/
...
-- Form goals from flat feature matrix columns
