/*
  # Fix ml_run_bias_refinement_simulation RPC — Correct Schema

  ## Changes
  - Uses model_lab.match_model_predictions (p_home/p_draw/p_away, competition_name, predicted_result)
  - Joins model_lab.match_model_evaluations on prediction_id for actual_result
  - Uses calibration_adjustments.group_key = competition_name (text match, not numeric API ID)
  - Competition health tracked by name: 'Premier League', 'Bundesliga', 'Serie A', 'La Liga', 'Ligue 1', 'Eredivisie', 'Sueper Lig'
  - Picks up raw baseline metrics from existing temp160_compbias_sigmoid_cap_010 simulation row
*/
...
DECLARE
  ...
  -- 7 competitions: PL, Bundesliga, Serie A, La Liga, Ligue 1, Eredivisie, Sueper Lig
...
BEGIN
  ...
  -- ── Mode → config ────────────────────────────────────────────────────────
  -- Family A: Sigmoid k tuning T→CB
  ...
  -- Family C: Hybrid sigmoid + relative T→CB
  ELSIF p_mode = 'temp160_sigmoid008_k075_relative20' THEN v_use_sigmoid:=true; v_use_relative:=true; v_sigmoid_cap:=0.08; v_sigmoid_k:=0.75; v_relative_cap:=0.20; v_family:='hybrid'; v_objective:='sigmoid 0.08/0.75 + relative 20%';
  ELSIF p_mode = 'temp160_sigmoid009_k075_relative20' THEN v_use_sigmoid:=true; v_use_relative:=true; v_sigmoid_cap:=0.09; v_sigmoid_k:=0.75; v_relative_cap:=0.20; v_family:='hybrid'; v_objective:='sigmoid 0.09/0.75 + relative 20%';
  ELSIF p_mode = 'temp160_sigmoid010_k075_relative20' THEN v_use_sigmoid:=true; v_use_relative:=true; v_sigmoid_cap:=0.10; v_sigmoid_k:=0.75; v_relative_cap:=0.20; v_family:='hybrid'; v_objective:='sigmoid 0.10/0.75 + relative 20%';
  -- Family D: CB→T pipeline
  ELSIF p_mode = 'robust_cb_sigmoid009_k075_then_temp160' THEN v_use_sigmoid:=true; v_cb_before_temp:=true; v_sigmoid_cap:=0.09; v_sigmoid_k:=0.75; v_family:='cb_then_t'; v_objective:='CB sigmoid 0.09/0.75 → T=1.6';
  ELSIF p_mode = 'robust_cb_sigmoid010_k075_then_temp160' THEN v_use_sigmoid:=true; v_cb_before_temp:=true; v_sigmoid_cap:=0.10; v_sigmoid_k:=0.75; v_family:='cb_then_t'; v_objective:='CB sigmoid 0.10/0.75 → T=1.6';
  ELSIF p_mode = 'robust_cb_relative20_then_temp160'       THEN v_use_relative:=true; v_cb_before_temp:=true; v_relative_cap:=0.20; v_family:='cb_then_t'; v_objective:='CB relative 20% → T=1.6';
  -- Family E: League ablations
  ELSIF p_mode = 'temp160_sigmoid010_k075_ligue1_half_draw_bias'         THEN v_use_sigmoid:=true; v_sigmoid_cap:=0.10; v_sigmoid_k:=0.75; v_ligue1_draw_half:=true;  v_family:='league_ablation'; v_objective:='sigmoid 0.10/0.75 + L1 draw÷2';
  ELSIF p_mode = 'temp160_sigmoid010_k075_bundesliga_half_away_bias'     THEN v_use_sigmoid:=true; v_sigmoid_cap:=0.10; v_sigmoid_k:=0.75; v_bundesliga_away_half:=true; v_family:='league_ablation'; v_objective:='sigmoid 0.10/0.75 + BL away÷2';
  ELSIF p_mode = 'temp160_sigmoid010_k075_l1_half_draw_bl_half_away'    THEN v_use_sigmoid:=true; v_sigmoid_cap:=0.10; v_sigmoid_k:=0.75; v_ligue1_draw_half:=true; v_bundesliga_away_half:=true; v_family:='league_ablation'; v_objective:='sigmoid 0.10/0.75 + L1 draw÷2 + BL away÷2';
  ELSE
    RETURN jsonb_build_object('error', 'unknown_mode', 'mode', p_mode);
  END IF;

  -- ── Load raw baseline from existing sim ─────────────────────────────────
  SELECT s.raw_avg_brier_1x2, s.raw_avg_log_loss_1x2, s.raw_result_accuracy,
         s.raw_pred_home_rate, s.raw_pred_draw_rate, s.raw_pred_away_rate,
         s.actual_home_rate, s.actual_draw_rate, s.actual_away_rate, s.sample_size
  INTO   v_raw_brier, v_raw_log_loss, v_raw_accuracy,
         v_raw_pred_home, v_raw_pred_draw, v_raw_pred_away,
         v_actual_home, v_actual_draw, v_actual_away, v_sample_size
  FROM   model_lab.calibration_adjustment_simulations s
  WHERE  s.source_backtest_run_id = p_run_id
    AND  s.simulation_key IN ('temp160_compbias_sigmoid_cap_010', 'temp160_compbias_cap_010')
  ...
  LOOP
    ...
    ELSE
      ...
      IF v_use_relative THEN
        -- For hybrid: sigmoid was already applied, now relative clips on T-scaled probs
      ...
      END IF;
      -- Add to T-scaled probs + normalize
    ...
    -- Brier
    IF    v_actual = 'H' THEN v_brier_i := (v_ph-1)^2 + v_pd^2 + v_pa^2;
    ELSIF v_actual = 'D' THEN v_brier_i := v_ph^2 + (v_pd-1)^2 + v_pa^2;
    ELSE                       v_brier_i := v_ph^2 + v_pd^2 + (v_pa-1)^2;
    END IF;
  ...
  FOR v_ii IN 1..7 LOOP
    IF v_comp_total[v_ii] > 0 THEN
      v_health_obj := jsonb_build_object(
        ...
        'accuracy_delta', round(((v_comp_correct[v_ii]::numeric - v_comp_raw_correct[v_ii]::numeric) / v_comp_total[v_ii]) * 100.0, 2),
  ...
  INSERT INTO model_lab.calibration_adjustment_simulations (
    ...
    brier_skill_vs_raw,
    simulation_verdict, rejection_flags,
    argmax_stability_json, per_competition_health_json,
    simulation_family, family_objective,
    sigmoid_k, relative_cap_pct, pipeline_order,
    bias_transform_config
  )
  VALUES (
    p_run_id, p_mode, 'completed', 'ml_run_bias_refinement_simulation',
    v_total,
    v_raw_brier, v_adj_brier,
    ...
    v_health_arr,
    v_family, v_objective,
    v_sigmoid_k, v_relative_cap,
  ...
  RETURN jsonb_build_object(
    ...
    'brier', round(v_adj_brier, 8), 'accuracy', round(v_adj_accuracy, 4),
