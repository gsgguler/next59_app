/*
  # Fix assess_upcoming_match_readiness — competition_name and season_label columns

  ## Problem
  model_lab.assess_upcoming_match_readiness references cs.competition_name and cs.season_label
  on the competition_seasons table, but those columns don't exist there.

  Correct source:
  - competition_name → competitions.name (via competition_seasons.competition_id)
  - season_label     → seasons.label     (via competition_seasons.season_id)

  ## Fix
  Add JOIN to public.competitions c and public.seasons s in the match-load query.
  Replace cs.competition_name → c.name, cs.season_label → s.label.
...
BEGIN
-- Load match — fixed: join competitions + seasons for name/label
SELECT
  m.*,
