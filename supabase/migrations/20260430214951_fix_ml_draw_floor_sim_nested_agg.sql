/*
  # Fix ml_run_draw_floor_simulation — nested aggregate error in dynamic floor query

  The COALESCE(jsonb_object_agg(...), ...) wrapping a GROUP BY subquery
  cannot be used directly as a scalar. Replace with a two-step approach:
  use a subquery CTE to compute draw rates first, then aggregate into jsonb.
*/
...
DECLARE
  ...
  v_temp          numeric := 0;
  v_use_comp_bias boolean := true;
  v_draw_floor    numeric := 0;
  ...
  v_total int:=0;
  v_raw_brier numeric:=0; v_adj_brier numeric:=0;
  v_raw_ll numeric:=0;    v_adj_ll numeric:=0;
  v_raw_correct int:=0;   v_adj_correct int:=0;
  ...
  v_floor numeric; v_draw_delta numeric;
...
BEGIN
  ...
  IF p_mode='draw_floor_12_plus_competition_bias' THEN v_draw_floor:=0.12;
  ELSIF p_mode='draw_floor_15_plus_competition_bias' THEN v_draw_floor:=0.15;
  ELSIF p_mode='draw_floor_18_plus_competition_bias' THEN v_draw_floor:=0.18;
  ELSIF p_mode='draw_floor_22_plus_competition_bias' THEN v_draw_floor:=0.22;
  ELSIF p_mode='dynamic_draw_floor_60pct_by_competition' THEN v_dynamic_floor:=true; v_dynamic_pct:=0.60; v_dynamic_min:=0.10; v_dynamic_max:=0.18;
  ELSIF p_mode='dynamic_draw_floor_70pct_by_competition' THEN v_dynamic_floor:=true; v_dynamic_pct:=0.70; v_dynamic_min:=0.12; v_dynamic_max:=0.22;
  ELSIF p_mode='temp_scale_15_plus_competition_bias' THEN v_temp:=1.5;
  ELSIF p_mode='temp_scale_20_plus_competition_bias' THEN v_temp:=2.0;
  ELSIF p_mode='temp_scale_15_plus_draw_floor_15' THEN v_temp:=1.5; v_draw_floor:=0.15;
  ELSIF p_mode='temp_scale_20_plus_draw_floor_15' THEN v_temp:=2.0; v_draw_floor:=0.15;
  ELSIF p_mode='temp_scale_15_plus_dynamic_draw_floor_70pct' THEN v_temp:=1.5; v_dynamic_floor:=true; v_dynamic_pct:=0.70; v_dynamic_min:=0.12; v_dynamic_max:=0.22;
  ELSIF p_mode='temp_scale_20_plus_dynamic_draw_floor_70pct' THEN v_temp:=2.0; v_dynamic_floor:=true; v_dynamic_pct:=0.70; v_dynamic_min:=0.12; v_dynamic_max:=0.22;
  ELSE RETURN jsonb_build_object('error','Unknown mode: '||p_mode);
  END IF;

  v_transform_cfg := jsonb_build_object('mode',p_mode,'temperature',v_temp,'draw_floor',v_draw_floor,
    'dynamic_floor',v_dynamic_floor,'dynamic_pct',v_dynamic_pct,'dynamic_min',v_dynamic_min,'dynamic_max',v_dynamic_max);

  -- Load competition bias
  SELECT
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('group_key',group_key,'adjustment_type',adjustment_type,'adjustment_value',adjustment_value)),'[]'::jsonb)
  INTO v_applied
  FROM model_lab.calibration_adjustments
  WHERE source_backtest_run_id=p_run_id AND group_type='competition' AND is_active=false;

  -- Load per-competition actual draw rates (fixed: subquery approach, no nested aggregate)
  IF v_dynamic_floor THEN
    SELECT jsonb_object_agg(competition_name, draw_rate)
    INTO v_comp_draw_rate
    FROM (
      SELECT p.competition_name,
             round(COUNT(*) FILTER (WHERE e.actual_result='D')::numeric / NULLIF(COUNT(*),0), 4) AS draw_rate
    ...
    v_comp_draw_rate := COALESCE(v_comp_draw_rate, '{}'::jsonb);
  END IF;
  ...
  LOOP
    v_total:=v_total+1;
    v_comp:=rec.competition_name; v_actual:=rec.actual_result;
    ...
    IF v_temp > 0 THEN
      v_logph:=ln(GREATEST(1e-9,v_ph)); v_logpd:=ln(GREATEST(1e-9,v_pd)); v_logpa:=ln(GREATEST(1e-9,v_pa));
      v_pah:=exp(v_logph/v_temp); v_pad:=exp(v_logpd/v_temp); v_paa:=exp(v_logpa/v_temp);
    ...
    v_pah:=LEAST(0.95,v_pah/v_norm); v_pad:=LEAST(0.95,v_pad/v_norm); v_paa:=LEAST(0.95,v_paa/v_norm);
    v_norm:=v_pah+v_pad+v_paa;
    v_pah:=v_pah/v_norm; v_pad:=v_pad/v_norm; v_paa:=v_paa/v_norm;

    -- Step 3: Draw floor
    IF v_dynamic_floor THEN
      v_floor:=GREATEST(v_dynamic_min,LEAST(v_dynamic_max,COALESCE((v_comp_draw_rate->>v_comp)::numeric,0.25)*v_dynamic_pct));
    ELSE
    ...
    v_conf:=jsonb_set(v_conf,ARRAY[v_conf_key],to_jsonb(COALESCE((v_conf->>v_conf_key)::int,0)+1),true);

    v_bin_idx:=CASE WHEN v_pad<0.10 THEN 1 WHEN v_pad<0.20 THEN 2 WHEN v_pad<0.30 THEN 3 WHEN v_pad<0.40 THEN 4 WHEN v_pad<0.50 THEN 5 ELSE 6 END;
    ...
    IF (GREATEST(v_pah,v_pad,v_paa)-(v_pah+v_pad+v_paa-GREATEST(v_pah,v_pad,v_paa)-LEAST(v_pah,v_pad,v_paa)))<0.07 THEN v_sc:='balanced'; END IF;
    IF v_pah BETWEEN 0.25 AND 0.45 AND v_pad BETWEEN 0.25 AND 0.45 AND v_paa BETWEEN 0.25 AND 0.45 THEN v_sc:='volatile'; END IF;
    v_scenario_counts:=jsonb_set(v_scenario_counts,ARRAY[v_sc],to_jsonb(COALESCE((v_scenario_counts->>v_sc)::int,0)+1),true);

    v_cacc:=COALESCE(v_per_comp->v_comp,jsonb_build_object('n',0,'adj_correct',0,'raw_correct',0,'adj_pred_h',0,'adj_pred_d',0,'adj_pred_a',0,'act_h',0,'act_d',0,'act_a',0));
  ...
  IF v_total=0 THEN RETURN jsonb_build_object('error','No predictions found'); END IF;
  ...
  FOR v_i IN 1..6 LOOP
    IF v_bin_n[v_i]>0 THEN
      ...
      bin_label:=CASE v_i WHEN 1 THEN '0.00-0.10' WHEN 2 THEN '0.10-0.20' WHEN 3 THEN '0.20-0.30' WHEN 4 THEN '0.30-0.40' WHEN 5 THEN '0.40-0.50' ELSE '0.50+' END;
      v_ece_draw:=v_ece_draw+(v_bin_n[v_i]::numeric/v_total)*bin_gap;
      v_bins_out:=v_bins_out||jsonb_build_object('bin',bin_label,'n',v_bin_n[v_i],'avg_pred_draw',round(bin_avg_pred,4),'actual_draw_rate',round(bin_act_rate,4),'gap',round(bin_gap,4));
    END IF;
  END LOOP;

  IF v_adj_brier/v_total > v_raw_brier/v_total+0.015 THEN v_rflags:=v_rflags||'["REJECTED: Brier worsens > 0.015"]'::jsonb; END IF;
  IF v_adj_ll/v_total > v_raw_ll/v_total+0.025 THEN v_rflags:=v_rflags||'["REJECTED: Log Loss worsens > 0.025"]'::jsonb; END IF;
  IF v_raw_correct::numeric/v_total - v_adj_correct::numeric/v_total > 0.02 THEN v_rflags:=v_rflags||'["REJECTED: Accuracy drops > 2pp"]'::jsonb; END IF;
  IF v_adj_pred_a::numeric/v_total < 0.08 THEN v_rflags:=v_rflags||'["REJECTED: Away rate < 8%"]'::jsonb; END IF;
  IF v_draw_prec < 0.25 THEN v_rflags:=v_rflags||'["REJECTED: Draw precision < 25%"]'::jsonb; END IF;
  IF v_adj_pred_h::numeric/v_total > 0.80 THEN v_rflags:=v_rflags||'["RISKY: Home rate > 80%"]'::jsonb; END IF;
  IF v_adj_pred_d::numeric/v_total < 0.12 THEN v_rflags:=v_rflags||'["RISKY: Draw rate < 12%"]'::jsonb; END IF;
  ...
  IF jsonb_array_length(v_rflags)=0 AND v_adj_brier/v_total<=v_raw_brier/v_total+0.002 AND v_adj_pred_d::numeric/v_total>=0.12 AND v_draw_f1>0.20 THEN
    v_verdict:='promising';
  ...
  v_notes:=v_notes||'Draw F1='||round(v_draw_f1,3)||'. Away F1='||round(v_away_f1,3)||'.';
  ...
  INSERT INTO model_lab.calibration_adjustment_simulations (
    ...
    probability_transform_config,rejection_flags,simulation_verdict,notes
  ) VALUES (
    ...
    v_raw_pred_h::numeric/v_total-v_adj_pred_h::numeric/v_total,v_conf,
    round(v_draw_prec,6),round(v_draw_rec,6),round(v_draw_f1,6),
    round(v_away_prec,6),round(v_away_rec,6),round(v_away_f1,6),
    round(v_ece_draw,6),v_bins_out,v_transform_cfg,v_rflags,v_verdict,NULLIF(trim(v_notes),'')
  )
  ON CONFLICT (source_backtest_run_id,simulation_key) DO UPDATE SET
    adjusted_avg_brier_1x2=EXCLUDED.adjusted_avg_brier_1x2,adjusted_avg_log_loss_1x2=EXCLUDED.adjusted_avg_log_loss_1x2,
    ...
    raw_decision_distribution_json=EXCLUDED.raw_decision_distribution_json,adjusted_decision_distribution_json=EXCLUDED.adjusted_decision_distribution_json,
    decision_rule_config=EXCLUDED.decision_rule_config,scenario_class_distribution_json=EXCLUDED.scenario_class_distribution_json,
    draw_capture_rate=EXCLUDED.draw_capture_rate,home_overcall_reduction=EXCLUDED.home_overcall_reduction,confusion_matrix_json=EXCLUDED.confusion_matrix_json,
    ...
    expected_calibration_error_draw=EXCLUDED.expected_calibration_error_draw,reliability_bins_draw=EXCLUDED.reliability_bins_draw,
  ...
  RETURN jsonb_build_object(
    'simulation_id',v_sim_id,'simulation_key',p_mode,'sample_size',v_total,'verdict',v_verdict,
    'raw_accuracy',round(v_raw_correct::numeric/v_total,6),'adj_accuracy',round(v_adj_correct::numeric/v_total,6),
    'raw_avg_brier',round(v_raw_brier/v_total,8),'adj_avg_brier',round(v_adj_brier/v_total,8),
    'raw_avg_ll',round(v_raw_ll/v_total,8),'adj_avg_ll',round(v_adj_ll/v_total,8),
    ...
    'draw_f1',round(v_draw_f1,6),'away_f1',round(v_away_f1,6),
    ...
    'ece_draw',round(v_ece_draw,6),'rejection_flags',v_rflags,'notes',COALESCE(trim(v_notes),'')
  );
END;
$$;
