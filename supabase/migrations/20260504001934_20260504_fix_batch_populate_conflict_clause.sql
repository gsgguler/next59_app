/*
  # Fix ambiguous column reference in ml_populate_feature_snapshot_batch_v1
  The ON CONFLICT DO UPDATE SET clause referenced unqualified column names
  that were ambiguous with PL/pgSQL variables. Fixed by using EXCLUDED.* prefix.
*/
...
BEGIN
  ...
  INSERT INTO model_lab.feature_snapshot_batch_runs AS br
  ...
  BEGIN
    ...
    CROSS JOIN LATERAL (
      SELECT
        ...
        ROUND(AVG(CASE WHEN prev.n <= 5  AND prev.pts = 3 THEN 1.0 WHEN prev.n <= 5  THEN 0.0 END)::numeric, 4) AS home_win_rate_l5,
        ROUND(AVG(CASE WHEN prev.n <= 5  AND prev.pts = 1 THEN 1.0 WHEN prev.n <= 5  THEN 0.0 END)::numeric, 4) AS home_draw_rate_l5,
        ROUND(AVG(CASE WHEN prev.n <= 5  AND prev.pts = 0 THEN 1.0 WHEN prev.n <= 5  THEN 0.0 END)::numeric, 4) AS home_loss_rate_l5,
        ROUND(AVG(CASE WHEN prev.n <= 10 AND prev.pts = 3 THEN 1.0 WHEN prev.n <= 10 THEN 0.0 END)::numeric, 4) AS home_win_rate_l10,
        ROUND(AVG(CASE WHEN prev.n <= 10 AND prev.pts = 1 THEN 1.0 WHEN prev.n <= 10 THEN 0.0 END)::numeric, 4) AS home_draw_rate_l10,
        ROUND(AVG(CASE WHEN prev.n <= 10 AND prev.pts = 0 THEN 1.0 WHEN prev.n <= 10 THEN 0.0 END)::numeric, 4) AS home_loss_rate_l10,
      ...
      FROM (
        ...
        LEFT JOIN public.match_stats ms
          ON ms.match_id = pm.id AND ms.team_id = u.home_team_id AND ms.half = 'FT'
    ...
    CROSS JOIN LATERAL (
      SELECT
        ...
        ROUND(AVG(CASE WHEN prev.n <= 5  AND prev.pts = 3 THEN 1.0 WHEN prev.n <= 5  THEN 0.0 END)::numeric, 4) AS away_win_rate_l5,
        ROUND(AVG(CASE WHEN prev.n <= 5  AND prev.pts = 1 THEN 1.0 WHEN prev.n <= 5  THEN 0.0 END)::numeric, 4) AS away_draw_rate_l5,
        ROUND(AVG(CASE WHEN prev.n <= 5  AND prev.pts = 0 THEN 1.0 WHEN prev.n <= 5  THEN 0.0 END)::numeric, 4) AS away_loss_rate_l5,
        ROUND(AVG(CASE WHEN prev.n <= 10 AND prev.pts = 3 THEN 1.0 WHEN prev.n <= 10 THEN 0.0 END)::numeric, 4) AS away_win_rate_l10,
        ROUND(AVG(CASE WHEN prev.n <= 10 AND prev.pts = 1 THEN 1.0 WHEN prev.n <= 10 THEN 0.0 END)::numeric, 4) AS away_draw_rate_l10,
        ROUND(AVG(CASE WHEN prev.n <= 10 AND prev.pts = 0 THEN 1.0 WHEN prev.n <= 10 THEN 0.0 END)::numeric, 4) AS away_loss_rate_l10,
      ...
      FROM (
        ...
        FROM public.matches pm
        LEFT JOIN public.match_stats ms
          ON ms.match_id = pm.id AND ms.team_id = u.away_team_id AND ms.half = 'FT'
    ...
    UPDATE model_lab.feature_snapshot_batch_runs AS br2
    SET status        = 'completed',
        rows_inserted = v_inserted,
        completed_at  = clock_timestamp()
    WHERE br2.batch_key = p_batch_key;
