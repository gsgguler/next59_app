/*
  # Fix generate_prematch_brain_package: created_at → generated_at

  The prematch_prediction_drafts table uses `generated_at` as its timestamp column,
  not `created_at`. The brain package function queried `ORDER BY created_at DESC`
  which caused a column-not-found error at runtime.

  Changes:
  - Replace both occurrences of `ORDER BY created_at DESC` with `ORDER BY generated_at DESC`
    inside model_lab.generate_prematch_brain_package
*/
...
v_p_home    := COALESCE((v_draft.generated_payload->>'p_home')::numeric,
               COALESCE(v_draft.p_home, 0.40));
v_p_draw    := COALESCE((v_draft.generated_payload->>'p_draw')::numeric,
               COALESCE(v_draft.p_draw, 0.28));
v_p_away    := COALESCE((v_draft.generated_payload->>'p_away')::numeric,
               COALESCE(v_draft.p_away, 0.32));
