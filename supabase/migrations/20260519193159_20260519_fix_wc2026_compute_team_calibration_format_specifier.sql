/*
  # Fix wc2026_compute_team_calibration — invalid format() specifier

  ## Problem
  PostgreSQL format() only supports %s, %I, %L — not C-style %.0f.
  The v_notes assignment used format('... %.0f ...', v_historical_elo, ...)
  which threw "unrecognized format() type specifier" at runtime, causing
  all 48 team calibrations to fail with status 'failed'.

  ## Fix
  Replace format() call with explicit text concatenation using round() and ::text.
  No logic changes — only the notes string construction is affected.

  ## Also fixes
  - Curaçao overall_status: the team has a verified api_football_team_id (5530)
    in wc2026_fixtures; it was incorrectly labelled 'missing_mapping'.
    The real issue is no WC history baseline, so status → 'pending'
    with a clear note preserved.
*/
...
-- ── Layer 2: Recent form (wc_history 2018/2022) ───────────────────────────
DECLARE
...
IF v_player_count > 0 THEN
  ...
  IF v_inj_count > 2 OR v_susp_count > 1 THEN
    v_warnings := v_warnings || ('["Kadro kaybı: ' || v_inj_count || ' sakatlık, ' || v_susp_count || ' ceza"]')::jsonb;
  END IF;
ELSE
  v_pos_coverage := 0;
...
IF v_bench_vs_xi IS NOT NULL THEN
  v_bench_impact := GREATEST(-1.0, LEAST(1.0, (v_bench_vs_xi - 0.6) * 2.5));
ELSE
  v_bench_impact := 0.0;
END IF;
...
-- FIX: use text concatenation instead of format() — PG format() does not support %.0f
...
-- Fix Curaçao: has verified api_football_team_id=5530 in wc2026_fixtures.
-- 'missing_mapping' was wrong label — the real issue is no WC history ELO baseline.
-- Keep missing_warning=true (accurate) but correct overall_status → 'pending'.
UPDATE public.wc2026_team_pool
SET
  overall_status = 'pending',
  notes = 'No WC history baseline (first-time qualifier) — calibration will use default ELO 1350. API mapping verified (id=5530).'
WHERE fifa_code = 'CUW'
  AND api_football_team_id = 5530;
