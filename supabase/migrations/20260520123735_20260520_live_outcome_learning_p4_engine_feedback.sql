/*
  Live Micro-Simulation — Outcome Learning V1: Phase 6 Engine Feedback

  Updates build_live_micro_windows to:
  1. Look up live_micro_pattern_memory after computing each window's state
  2. Apply confidence_adjustment (additive, capped ±0.10)
  3. Set reliability_warning from low_sample_warning or high false rates
  4. Add historical_reliability to reasoning_json

  Raw pressure/chaos scores are NOT changed.
  Only confidence and warning columns are adjusted.
*/
...
DECLARE
  ...
  -- Pattern memory feedback vars
  v_minute_bucket      text;
  v_score_state        text;
  v_pressure_bucket    text;
  v_pm                 record;
  v_conf_adjustment    numeric;
  v_rel_warning        text;
BEGIN
  ...
  FOR v_w_start IN SELECT generate_series(0, 90, 5) LOOP
    ...
    SELECT
      COUNT(*)::integer,
      COUNT(*) FILTER (WHERE event_type='Goal' AND api_football_team_id = v_home_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Goal' AND api_football_team_id = v_away_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Card' AND event_detail NOT ILIKE '%red%' AND api_football_team_id = v_home_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Card' AND event_detail NOT ILIKE '%red%' AND api_football_team_id = v_away_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Card' AND event_detail ILIKE '%red%' AND api_football_team_id = v_home_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Card' AND event_detail ILIKE '%red%' AND api_football_team_id = v_away_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='subst' AND api_football_team_id = v_home_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='subst' AND api_football_team_id = v_away_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Var' AND api_football_team_id = v_home_team_id)::integer,
      COUNT(*) FILTER (WHERE event_type='Var' AND api_football_team_id = v_away_team_id)::integer
    INTO
    ...
    FROM public.api_football_fixture_events
    WHERE api_football_fixture_id = p_fixture_id
      AND elapsed >= v_w_start AND elapsed < v_w_end;
    ...
    ELSE
      v_shots_h_win := NULL; v_shots_a_win := NULL;
      v_poss_h_win  := NULL; v_poss_a_win  := NULL;
    END IF;
    ...
    v_fatigue_wave := CASE
      ...
      ELSE 0.75
    END;
    v_fatigue_wave := LEAST(1.0, v_fatigue_wave + (COALESCE(v_s_home,0) + COALESCE(v_s_away,0)) * 0.05);
    ...
    IF v_home_score < v_away_score AND v_w_start >= 60 THEN
      v_comeback := LEAST(1.0, v_pressure_home * 0.6 + (v_away_score - v_home_score) * 0.15
        + (CASE WHEN v_w_start >= 75 THEN 0.20 ELSE 0 END));
    ELSIF v_away_score < v_home_score AND v_w_start >= 60 THEN
      v_comeback := LEAST(1.0, v_pressure_away * 0.6 + (v_home_score - v_away_score) * 0.15
        + (CASE WHEN v_w_start >= 75 THEN 0.20 ELSE 0 END));
    END IF;

    v_draw_pres := CASE
      WHEN v_home_score = v_away_score AND v_w_start >= 70 THEN
        LEAST(1.0, 0.40 + v_fatigue_wave * 0.30 + (CASE WHEN v_w_start >= 85 THEN 0.20 ELSE 0 END))
    ...
    -- ── Pattern memory feedback (Phase 6) ────────────────────────────────────
    v_minute_bucket := CASE
      WHEN v_w_start < 15 THEN '0-15'
      WHEN v_w_start < 30 THEN '15-30'
      WHEN v_w_start < 45 THEN '30-45'
      WHEN v_w_start < 60 THEN '45-60'
      WHEN v_w_start < 75 THEN '60-75'
      ELSE '75-90'
    END;
    v_score_state := CASE
      WHEN v_comeback > 0.3 AND v_home_score < v_away_score THEN 'trailing'
      WHEN v_comeback > 0.3 AND v_away_score < v_home_score THEN 'trailing'
      WHEN v_home_score = v_away_score THEN 'level'
      ELSE 'leading'
    END;
    v_pressure_bucket := CASE
      WHEN v_pressure_delta > 0.20 THEN 'high_home'
      WHEN v_pressure_delta < -0.20 THEN 'high_away'
      ELSE 'balanced'
    END;

    v_conf_adjustment := 0.0;
    v_rel_warning := NULL;

    SELECT confidence_adjustment, low_sample_warning, sample_size,
           false_pressure_rate, false_chaos_rate, false_late_goal_rate,
           reliability_score
    INTO v_pm
    FROM model_lab.live_micro_pattern_memory
    WHERE micro_state = v_micro_state
      AND minute_bucket = v_minute_bucket
      AND score_state = v_score_state
      AND pressure_bucket = v_pressure_bucket
      AND source_quality_bucket = v_source_qual
    LIMIT 1;

    IF v_pm IS NOT NULL THEN
      v_conf_adjustment := COALESCE(v_pm.confidence_adjustment, 0.0);
      v_confidence := GREATEST(0.0, LEAST(1.0, v_confidence + v_conf_adjustment));

      IF v_pm.low_sample_warning THEN
        v_rel_warning := format('Düşük örnek (%s) — geçmiş örüntü güvenilir değil', v_pm.sample_size);
      ELSIF COALESCE(v_pm.false_pressure_rate, 0) > 0.5 THEN
        v_rel_warning := format('Yüksek yanlış baskı sinyali (%.0f%%) — tarihsel olarak yanıltıcı', v_pm.false_pressure_rate * 100);
      ELSIF COALESCE(v_pm.false_chaos_rate, 0) > 0.5 THEN
        v_rel_warning := format('Yüksek yanlış kaos sinyali (%.0f%%) — tarihsel olarak yanıltıcı', v_pm.false_chaos_rate * 100);
      ELSIF COALESCE(v_pm.false_late_goal_rate, 0) > 0.5 THEN
        v_rel_warning := format('Yüksek yanlış geç gol sinyali (%.0f%%) — tarihsel olarak yanıltıcı', v_pm.false_late_goal_rate * 100);
      END IF;

      IF v_conf_adjustment <> 0.0 THEN
        v_penalties := array_append(v_penalties,
          format('pattern_memory_adj:%s', v_conf_adjustment));
      END IF;
    END IF;
    ...
    v_reasoning := jsonb_build_object(
      ...
      'historical_reliability',       CASE WHEN v_pm IS NOT NULL THEN v_pm.reliability_score ELSE NULL END,
      'pattern_memory_adjustment',    v_conf_adjustment,
    ...
    IF v_existing_id IS NOT NULL THEN
      UPDATE model_lab.live_micro_windows SET
        ...
        reliability_warning=v_rel_warning
  ...
  UPDATE model_lab.live_micro_window_runs SET
    ...
    windows_created=v_windows_created, windows_updated=v_windows_updated,
