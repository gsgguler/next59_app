/*
  # Fix generate_prematch_brain_package: standings_readiness record type mismatch

  ## Problem
  `generate_prematch_brain_package` uses `SELECT * INTO v_readiness FROM model_lab.upcoming_match_readiness`.
  The `record` variable type was compiled before `standings_readiness`, `injuries_readiness`,
  `team_statistics_readiness`, `venue_readiness`, and `enrichment_score` columns were added
  (migration 20260518202448). PostgreSQL caches the row type at compile time, so the function
  throws "could not identify column standings_readiness in record data type" on every call.

  ## Fix
  DROP + CREATE REPLACE forces PostgreSQL to recompile and bind the current table row type.
  Function logic is identical — only the recompilation is needed.

  ## Impact
  - Fixes all 16 daily pipeline errors (step: brain)
  - prematch_pipeline last_success_at will start populating on next run
  - No data loss, no breaking changes
*/

DROP FUNCTION IF EXISTS model_lab.generate_prematch_brain_package(uuid, text);
...
-- Load readiness — typed as the actual row type to avoid record caching issues
...
-- DATA QUALITY BRAIN (uses typed row columns directly — no record casting)
...
-- Grant execute to authenticated (matches existing pattern)
