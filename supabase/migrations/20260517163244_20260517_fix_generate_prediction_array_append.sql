...
/*
  # Fix generate_prematch_prediction — use array_append() for all warning appends

  Replaces v_warnings := v_warnings || 'string' with
  v_warnings := array_append(v_warnings, 'string') throughout the function
  to avoid the text || text ambiguity that causes malformed array literal errors.
*/
...
BEGIN
  ...
  -- ELO: try team_elo_snapshots first (pre-match ELO at time of that fixture)
  ...
  IF v_cal_brier IS NOT NULL AND v_cal_brier > 0.26 THEN
    v_confidence := v_confidence - 0.05;
    v_warnings := array_append(v_warnings,
      format('brier %.3f > 0.26; confidence reduced', v_cal_brier));
  END IF;
