/*
  # Fix mapping engine v4: cross-product norm matching

  ## Problem
  When home team needs raw norm and away team needs alias norm (or vice versa),
  the engine misses the match. e.g. Bayern Munich (raw=direct match, ali=munchen variant)
  vs FC Heidenheim (raw=heidenheim, ali=fc heidenheim) — requires (raw_home, ali_away).

  ## Fix
  Add two additional matching passes: (ali, raw) and (raw, ali) for both exact and ±1day.
*/
...
-- ── Exact date: all 4 norm combinations ──────────────────────────────────────
DO $inner$
DECLARE
  _combos text[][] := ARRAY[
