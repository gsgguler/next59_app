/*
  # Fix ml_run_pathology_simulation — use array_append for text[] flags

  The previous version used `v_reject_flags || 'string'` which fails in PL/pgSQL
  because the || operator on text[] requires array || array, not array || scalar.
  This patch replaces all occurrences with array_append().
*/
