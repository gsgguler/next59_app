/*
  # Live Micro-Simulation Engine V1 — Builder + State Machine

  ## Summary
  `model_lab.build_live_micro_windows(p_fixture_id)` — deterministic 5-minute
  window engine. Reads api_football_fixture_events, computes pressure/chaos/state
  signals per window, upserts into live_micro_windows.

  No LLM. No fake xG. No fabricated possession. Event-only when stats absent.

  ## State Machine
  Separate IF/ELSIF chain so reasoning is captured per rule.
*/

CREATE OR REPLACE FUNCTION model_lab.build_live_micro_windows(
  p_fixture_id bigint
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Resolve home/away team IDs (most/least frequent in events)
  WITH team_counts AS (
    SELECT api_football_team_id AS tid, COUNT(*) AS cnt
    FROM api_football_fixture_events
  ...
  -- Extract end-of-match stats from raw JSON
  BEGIN
    SELECT
    ...
    FROM (
      ...
      FROM api_football_fixture_statistics_raw,
  ...
  EXCEPTION WHEN OTHERS THEN
    v_has_stats := false;
    v_warnings := v_warnings || ('stats_extraction_failed: ' || SQLERRM);
  END;

  -- Window loop: 0,5,10,...,90 → windows [0-5), [5-10), ..., [90-95)
  FOR v_w_start IN SELECT generate_series(0, 90, 5) LOOP
    ...
    -- Distribute stats proportionally
    IF v_has_stats THEN
    ...
    -- State machine (explicit IF chain — reasoning captured per rule)
    ...
    IF v_home_team_id IS NULL THEN
      ...
      v_penalties := v_penalties || 'no_team_id:-0.30';
    END IF;
    IF NOT v_has_stats THEN
      v_confidence := v_confidence - 0.10;
      v_penalties := v_penalties || 'no_stats:-0.10';
    END IF;
    IF v_ev_count = 0 THEN
      v_confidence := v_confidence - 0.30;
      v_penalties := v_penalties || 'no_events:-0.30';
    ELSIF v_ev_count <= 1 THEN
      v_confidence := v_confidence - 0.10;
      v_penalties := v_penalties || 'sparse_events:-0.10';
    END IF;
    ...
    IF NOT v_has_stats    THEN v_missing := v_missing || 'fixture_statistics_unavailable'; END IF;
    IF v_home_team_id IS NULL THEN v_missing := v_missing || 'home_team_id_unknown'; END IF;
    IF v_away_team_id IS NULL THEN v_missing := v_missing || 'away_team_id_unknown'; END IF;
    ...
    ELSE
      INSERT INTO model_lab.live_micro_windows(
        ...
        v_home_score, v_away_score, v_ev_count, COALESCE(v_g_home,0), COALESCE(v_g_away,0),
        v_shots_h_win, v_shots_a_win, COALESCE(v_corner_h,0), COALESCE(v_corner_a,0),
...
-- Admin RPC wrapper
CREATE OR REPLACE FUNCTION public.admin_build_micro_windows(p_fixture_id bigint)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  RETURN model_lab.build_live_micro_windows(p_fixture_id);
END;
$$;

GRANT EXECUTE ON FUNCTION public.admin_build_micro_windows(bigint) TO authenticated;
