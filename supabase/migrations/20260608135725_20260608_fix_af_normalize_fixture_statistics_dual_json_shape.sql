/*
  # Fix af_normalize_fixture_statistics — dual JSON shape support

  The live ingest path (af-fixture-statistics) stores raw API envelope:
    { "get": "...", "response": [...], "results": N, ... }

  The backfill path (af-statistics-backfill) stores:
    { "fixture_id": N, "stats": [...] }

  The function previously only accepted the 'stats' key, silently skipping
  all 1,271 rows ingested via the live path. This migration adds 'response'
  key support while preserving all existing UPDATE logic unchanged.
*/
...
BEGIN
  ...
  FOR rec IN
    SELECT
      ...
      r.response_json,
      COALESCE(r.response_json->'stats', r.response_json->'response', '[]'::jsonb) AS stats_source,
      m.home_team_id,
      m.away_team_id
    ...
    WHERE r.transform_status = 'raw'
      ...
      AND (r.response_json ? 'stats' OR r.response_json ? 'response')
  LOOP
    BEGIN
      stats_source := rec.stats_source;

      -- index 0 = home team, index 1 = away team
      FOR team_idx IN 0..1 LOOP
        team_entry := stats_source->team_idx;
