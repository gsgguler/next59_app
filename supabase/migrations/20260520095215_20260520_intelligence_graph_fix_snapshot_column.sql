/*
  # Intelligence Graph — Fix feature snapshot column name

  The prematch_feature_matrix_snapshot_v1 table uses `snapshot_run_key` (not
  `feature_version`) and `snapshot_created_at` (not a generic created_at).
  This migration replaces the rebuild function with corrected column references.
*/
...
BEGIN
  ...
  IF p_scope <> 'wc2026' THEN
    ...
    LOOP
      INSERT INTO model_lab.intelligence_graph_nodes (
        ...
        calibration_version, status, confidence_score, updated_at, metadata_json
      ) VALUES (
        'prediction', 'model_lab', 'prematch_prediction_drafts',
        r_pred.id::text, r_pred.match_id,
        r_pred.model_version, r_pred.feature_version, r_pred.calibration_version,
        r_pred.status, r_pred.confidence_score, now(),
      ...
      ON CONFLICT (entity_schema, entity_table, entity_id) DO UPDATE
        SET status = EXCLUDED.status, confidence_score = EXCLUDED.confidence_score,
            updated_at = now();
      v_nodes_upserted := v_nodes_upserted + 1;
    END LOOP;
  END IF;

  -- ─── Feature snapshots (uses snapshot_run_key, snapshot_created_at) ──────
  IF p_scope <> 'wc2026' THEN
    FOR r_snap IN
      SELECT match_id::text AS entity_id, match_id,
             snapshot_run_key, data_quality_tier, leakage_check_passed,
             snapshot_created_at
    ...
    LOOP
      INSERT INTO model_lab.intelligence_graph_nodes (
        ...
        r_snap.snapshot_run_key, 'ready', now(),
        jsonb_build_object(
  ...
  IF p_scope <> 'wc2026' THEN
    ...
    LOOP
      INSERT INTO model_lab.intelligence_graph_nodes (
        ...
        r_brain.model_version, r_brain.feature_version, r_brain.status, now()
      )
      ON CONFLICT (entity_schema, entity_table, entity_id) DO UPDATE
  ...
  IF p_scope <> 'wc2026' THEN
    FOR r_pub IN
      SELECT id, match_id, story_draft_id, prediction_draft_id,
             is_visible, model_version, feature_version, calibration_version,
             published_at
    ...
    LOOP
      INSERT INTO model_lab.intelligence_graph_nodes (
        ...
        r_pub.model_version, r_pub.feature_version, r_pub.calibration_version,
  ...
  IF p_scope <> 'wc2026' THEN
    ...
    LOOP
      INSERT INTO model_lab.intelligence_graph_nodes (
        ...
        r_eval.id::text, r_eval.match_id, r_eval.confidence_score,
        CASE WHEN r_eval.was_correct THEN 'correct' ELSE 'incorrect' END,
        now(), jsonb_build_object('brier_score', r_eval.brier_score,
  ...
  IF p_scope <> 'wc2026' THEN
    ...
    LOOP
      INSERT INTO model_lab.intelligence_graph_nodes (
        ...
        r_live.id::text, r_live.match_id, r_live.state_confidence,
  ...
  -- prediction → feature_snapshot (depends_on_feature, same match_id)
  IF p_scope <> 'wc2026' THEN
    ...
    -- brain_run → prediction (generated_from)
    INSERT INTO model_lab.intelligence_graph_edges
      (from_node_id, to_node_id, relationship_type)
    ...
    -- story_draft → prediction (generated_from via prediction_draft_id)
    INSERT INTO model_lab.intelligence_graph_edges
      (from_node_id, to_node_id, relationship_type)
    ...
    -- publication → story_draft (published_as)
    INSERT INTO model_lab.intelligence_graph_edges
      (from_node_id, to_node_id, relationship_type)
    ...
    -- evaluation → prediction (evaluated_by)
    INSERT INTO model_lab.intelligence_graph_edges
      (from_node_id, to_node_id, relationship_type)
    ...
    -- live_outcome → prediction (linked_prediction, same match)
    INSERT INTO model_lab.intelligence_graph_edges
      (from_node_id, to_node_id, relationship_type)
  ...
  -- ─── Orphan warnings ──────────────────────────────────────────────────────
  SELECT count(*) INTO v_n FROM model_lab.intelligence_graph_nodes n
