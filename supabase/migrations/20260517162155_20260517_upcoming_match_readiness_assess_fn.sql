...
/*
  # Upcoming Match Readiness — Assessment Function

  ## Purpose
  `model_lab.assess_upcoming_match_readiness(p_match_id uuid)`

  Computes the full readiness profile for a single match and upserts into
  `model_lab.upcoming_match_readiness`. Idempotent — safe to call repeatedly.

  ## Readiness dimensions
  - elo_readiness          — both teams have ELO in team_elo_ratings (elo_v2_ha0_k20_global)
  - feature_readiness      — row exists in match_feature_matrix_v2
  - calibration_readiness  — league_calibration_state row exists for competition
  - lineup_availability    — af.fixture_lineups row exists (graceful if schema absent)
  - stats_availability     — prematch_feature_matrix_snapshot_v1.has_stats = true
  - prediction_readiness   — prematch_prediction_drafts row exists
  - scenario_readiness     — match_story_drafts row exists

  ## Overall status
  - 'ready'   — elo AND feature AND calibration all ready
  - 'partial' — elo AND (feature OR calibration)
  - 'blocked' — otherwise

  ## Batch wrapper
  `model_lab.assess_readiness_batch(competition_name, from_date, to_date)`
*/
...
DECLARE
  ...
  v_away_elo_cnt  int;
BEGIN
  -- Fetch core match row
  SELECT * INTO v_match FROM public.matches WHERE id = p_match_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'match_id % not found in public.matches', p_match_id;
  END IF;

  -- Resolve competition name via competition_seasons -> competitions
  ...
  -- === ELO READINESS ===
  SELECT COUNT(*), MAX(current_elo)
  INTO v_home_elo_cnt, v_elo_home
  FROM model_lab.team_elo_ratings
  WHERE team_id = v_match.home_team_id
    AND elo_version = 'elo_v2_ha0_k20_global'
    AND rated_through >= v_match.match_date - INTERVAL '45 days';
  ...
  IF NOT v_elo_ready THEN
    v_blocking := v_blocking || 'ELO ratings missing or stale for one or both teams';
  END IF;
  ...
  -- === LINEUP AVAILABILITY ===
  -- Graceful probe: af.fixture_lineups may not exist yet for all seasons
  BEGIN
  ...
  VALUES (
    ...
    CASE WHEN v_match.timestamp IS NOT NULL
         THEN to_timestamp(v_match.timestamp) ELSE NULL END,
...
-- Batch assessment over a competition/date range
CREATE OR REPLACE FUNCTION model_lab.assess_readiness_batch(
  p_competition_name text DEFAULT NULL,
  p_from_date        date DEFAULT CURRENT_DATE - INTERVAL '7 days',
  p_to_date          date DEFAULT CURRENT_DATE + INTERVAL '60 days'
)
RETURNS TABLE (
  match_id       uuid,
  home_team_name text,
...
BEGIN
  FOR v_mid IN
    ...
    WHERE m.match_date BETWEEN p_from_date AND p_to_date
      AND (p_competition_name IS NULL OR c.name = p_competition_name)
    ORDER BY m.match_date
  LOOP
    v_result := model_lab.assess_upcoming_match_readiness(v_mid);
    RETURN QUERY
      SELECT
        v_result.match_id,
        v_result.home_team_name,
        v_result.away_team_name,
        v_result.match_date,
        v_result.overall_status,
        cardinality(v_result.warnings)::int;
  END LOOP;
END;
$$;

GRANT EXECUTE ON FUNCTION model_lab.assess_readiness_batch(text, date, date) TO authenticated;
