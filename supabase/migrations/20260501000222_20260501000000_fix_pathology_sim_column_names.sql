/*
  # Fix ml_run_pathology_simulation — correct column names

  The previous version used incorrect column names (verdict, brier_adjusted, etc.).
  The actual table uses: simulation_verdict, adjusted_avg_brier_1x2,
  adjusted_result_accuracy, adjusted_pred_*_rate, reliability_bins_draw, etc.
  This replaces the function with correct column references.
*/
  ...
  BEGIN
    ...
    LOOP
      ...
      IF p_mode IN ('temp160_compbias_no_ligue1_draw_bias',
                    'temp160_compbias_cap008_no_ligue1_draw_bias',
                    'temp160_compbias_multiplier_no_ligue1_draw_bias')
         AND r.competition_name = 'Ligue 1' THEN dd := 0;
      END IF;
      ...
      ELSIF p_mode IN ('temp160_compbias_multiplier_prior',
        ...
        use_mult := true; tdh := 0; tdd := 0; tda := 0;
      ...
      ELSIF p_mode = 'temp160_compbias_entropy_scaled_additive' THEN
        ...
        tdh := dh * bias_scale; tdd := dd * bias_scale; tda := da * bias_scale;
  ...
  BEGIN
    ...
    FOR i IN 1..6 LOOP
      IF v_bin_count[i] > 0 THEN
        ...
        sx  := sx  + bxi; sy  := sy  + byi;
        sxy := sxy + bxi * byi; sxx := sxx + bxi * bxi;
      END IF;
    END LOOP;
    IF n_bins >= 2 THEN
    ...
    IF v_brier > c_raw_brier THEN
      v_reject_flags := array_append(v_reject_flags, 'brier_worse_than_raw');
    END IF;
    ...
    IF v_acc < (c_src_acc - 2.0) THEN
      v_reject_flags := array_append(v_reject_flags, 'accuracy_drop_over_2pp');
    END IF;
    IF (100.0 * v_pred_a / NULLIF(v_total,0)) < 8.0 THEN
      v_reject_flags := array_append(v_reject_flags, 'away_rate_below_8pct');
    END IF;
    IF v_draw_prec < 25.0 THEN
      v_reject_flags := array_append(v_reject_flags, 'draw_precision_below_25pct');
    END IF;
    IF v_l1_pct > 40.0 THEN
      v_reject_flags := array_append(v_reject_flags, 'ligue1_pred_draw_over_40pct');
    END IF;
    IF v_bl_delta < -2.0 THEN
      v_reject_flags := array_append(v_reject_flags, 'bundesliga_acc_drop_over_2pp');
    END IF;
    IF (100.0 * v_changed / NULLIF(v_total,0)) > 45.0 THEN
      v_risky_flags := array_append(v_risky_flags, 'argmax_changed_rate_over_45pct');
    END IF;
    ...
    VALUES (
      ...
      p_mode,
      verdict_text,
      'Pathology simulation: ' || p_mode,
      v_total,
      ROUND(v_brier::numeric, 8),
      ROUND(v_ll::numeric, 8),
      ROUND(v_acc::numeric, 4),
