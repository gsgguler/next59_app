...
function TeamPastWC({
  ...
  useEffect(() => {
    supabase
      .from("wch_matches")
      .select(
        "id,edition_year,stage_code,group_name,match_date,home_team_name,away_team_name,home_score_ft,away_score_ft,home_score_90,away_score_90,decided_by,home_score_aet,away_score_aet,home_penalties,away_penalties,final_winner_name,result_90",
      )
    ...
    return (
      <div className="space-y-2">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-10 bg-navy-800/30 rounded-lg animate-pulse"
          />
        ))}
      </div>
    );
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-slate-300">
          Dünya Kupası geçmiş kaydı bulunamadı.
        </p>
  ...
  const totalM = matches.filter(
    (m) => m.final_winner_name && m.final_winner_name !== dbName,
  ).length;
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        {[
          ...
          <div
            key={s.label}
            className="flex-1 text-center bg-navy-900/60 border border-navy-800 rounded-lg py-2"
          >
      ...
      <div className="space-y-4">
        {[...byEdition.entries()]
          ...
          .map(([year, ems]) => (
            <div key={year}>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5 px-0.5">
                {year} Dünya Kupası
              </p>
              <div className="space-y-1">
                {ems
                  .sort(
                    (a, b) =>
                      stageOrder(a.stage_code) - stageOrder(b.stage_code),
                  )
                  .map((m) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`shrink-0 w-5 h-5 rounded text-xs font-bold flex items-center justify-center border ${outcomeClass}`}
                        >
                          {!winner ? "B" : winner === dbName ? "G" : "M"}
                        </span>
                        <span className="text-xs text-slate-400 shrink-0 w-14">
                          {STAGE_LABELS[m.stage_code] ?? m.stage_code}
                        </span>
                        ...
                        <div className="shrink-0 flex items-center gap-1">
                          ...
                          {badge && (
                            <span className="text-xs text-champagne/80 font-medium">
                              {badge === "PEN" ? "Pen." : "Uzatma"}
                            </span>
                          )}
                        </div>
  ...
  if (meetings.length === 0)
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-slate-300">
          Bu iki takım daha önce Dünya Kupası'nda karşılaşmamış.
        </p>
      ...
      <div className="space-y-1.5">
        {meetings.map((m) => {
          ...
          return (
            ...
            >
              <span className="text-xs font-bold text-slate-300 shrink-0 w-8">
                {m.edition_year}
              </span>
              ...
              <span
                className={`flex-1 text-xs text-right truncate ${winner === m.home_team_name ? "text-white font-semibold" : "text-slate-400"}`}
              >
                {m.home_team_name}
              </span>
              ...
              <span
                className={`flex-1 text-xs truncate ${winner === m.away_team_name ? "text-white font-semibold" : "text-slate-400"}`}
              >
                {m.away_team_name}
              </span>
              <ChevronRight className="w-3 h-3 text-navy-700 group-hover:text-champagne transition-colors shrink-0" />
...
function GroupContext({
  ...
  allFixtures: WC2026Fixture[];
}) {
  if (!fixture.group) return null;
  const groupFixtures = allFixtures.filter(
    (f) => f.group === fixture.group && f.stage === "Group Stage",
  );
  const teams = [
    ...new Set([
          ...
          return (
            ...
            >
              <span className="text-xs text-slate-400 shrink-0 w-20">
                {formatMatchDateTime(f.kickoff_utc, userTZ)}
              </span>
              <span className="flex items-center gap-1.5 flex-1 justify-end">
                {hc && (
                  <span
                    className={`fi fi-${hc.iso2} rounded-[3px] shrink-0`}
                    style={{ width: 16, height: 11, display: "inline-block" }}
                  />
                )}
                <span
                  className={`text-xs truncate ${isCurrent ? "text-white font-semibold" : "text-slate-300"}`}
                >
                  {hc?.name_tr ?? f.home_team}
                </span>
              </span>
              ...
              <span className="flex items-center gap-1.5 flex-1">
                {ac && (
                  <span
                    className={`fi fi-${ac.iso2} rounded-[3px] shrink-0`}
                    style={{ width: 16, height: 11, display: "inline-block" }}
                  />
                )}
                <span
                  className={`text-xs truncate ${isCurrent ? "text-white font-semibold" : "text-slate-300"}`}
                >
                  {ac?.name_tr ?? f.away_team}
                </span>
              </span>
              {isCurrent && (
                <span className="text-xs text-champagne shrink-0">
                  ← Bu Maç
                </span>
              )}
            </div>
            ...
            return (
              <span
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs border ${
                  isCurrent
                    ? "bg-champagne/10 border-champagne/30 text-white font-semibold"
                    : "bg-navy-900/60 border-navy-800 text-slate-400"
                }`}
              >
                <span
                  className={`fi fi-${c.iso2} rounded-[2px]`}
...
function RiskBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded border font-medium ${cls}`}
    >
...
function Wc5MinFlowPanel({
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then((res) => {
        if (res?.data) setPrematchRows(res.data as FlowPeriodRow[]);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [fixtureUuid, isTBD]);
  ...
  useEffect(() => {
    ...
    const fetchLive = () => {
      supabase
        ...
        .select(
          "status_short,status_long,elapsed_minute,home_score,away_score,synced_at",
        )
        .eq("fixture_id", fixtureUuid)
        .maybeSingle()
        .then((res) => {
          if (res?.data) setLiveState(res.data as LiveMatchState);
        })
        .catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    const fetchLiveScenarios = () => {
      supabase
        ...
        .select(
          "period_start,period_end,live_minute,home_score,away_score,momentum_side,goal_risk_home,goal_risk_away,card_risk,corner_risk,foul_intensity,narrative_text,generated_at",
        )
        .eq("fixture_id", fixtureUuid)
        .eq("is_current", true)
        .order("period_start", { ascending: true })
        .then((res) => {
          if (res?.data) setLiveRows(res.data as LiveFlowRow[]);
        })
        .catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .maybeSingle()
      .then((res) => {
        if (res?.data?.key_match_triggers)
          setKeyTriggers(res.data.key_match_triggers as string[]);
      });
  }, [apiFootballFixtureId, isTBD]);

  const isLive =
    liveState !== null && LIVE_STATUSES.has(liveState.status_short);
  const isFinished =
    liveState !== null && FINISHED_STATUSES.has(liveState.status_short);
        ...
        <div className="flex items-center gap-2.5">
          ...
          {isLive && (
            <span className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded bg-red-900/50 border border-red-700/50 text-red-300 font-bold animate-pulse">
              ...
              CANLI{" "}
              {liveState.elapsed_minute ? `${liveState.elapsed_minute}'` : ""}
            </span>
          )}
          {isFinished && !hideFinishedResult && liveState && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-700/50 border border-slate-600/40 text-slate-300 font-mono">
              ...
              {liveState.status_short}
            </span>
          )}
          {!isLive && !isFinished && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-navy-700 text-navy-300 font-mono">
              v{ver}
            </span>
          )}
        </div>
        <ChevronDown
          className={`w-4 h-4 text-navy-400 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`}
        />
      </button>
        ...
        <div className="px-4 pb-4 pt-3 space-y-3 bg-navy-900/20">
          ...
          {(isLive || (isFinished && !hideFinishedResult)) && liveState && (
            <div className="flex items-center justify-center gap-3 py-2.5 bg-navy-800/50 rounded-lg border border-navy-700/50">
              ...
              <span
                className={`text-xs font-semibold px-2 py-0.5 rounded ${isLive ? "bg-red-900/50 text-red-300" : "bg-slate-700/50 text-slate-300"}`}
              >
                {liveState.status_long ?? liveState.status_short}
                {isLive && liveState.elapsed_minute
                  ? ` ${liveState.elapsed_minute}'`
                  : ""}
              </span>
            </div>
          )}
          ...
          {showLiveFlow && (
            <>
              <p className="text-[11px] text-navy-500 leading-relaxed">
                Canlı maç verilerine dayalı anlık akış analizi. Her 2 dakikada
                bir güncellenir.
              </p>
              <div className="space-y-0.5">
                {liveRows.map((row) => {
                  ...
                  return (
                    <div
                      key={`${row.period_start}-${row.live_minute}`}
                      className="flex flex-col gap-1 px-2.5 py-2 rounded-md bg-navy-800/25 border border-white/[0.04] hover:bg-navy-800/40 transition-colors"
                    >
                      <div className="flex items-center gap-2 flex-wrap">
                        <span
                          className={`shrink-0 text-[11px] font-bold font-mono w-10 ${col}`}
                        >
                          {row.period_start}'
                        </span>
                        <MomentumDot side={row.momentum_side} />
                        <div className="flex items-center gap-1 flex-wrap">
                          {row.goal_risk_home > 0.04 && (
                            <RiskBadge
                              value={row.goal_risk_home}
                              label="Gol H"
                              color="amber"
                            />
                          )}
                          {row.goal_risk_away > 0.04 && (
                            <RiskBadge
                              value={row.goal_risk_away}
                              label="Gol D"
                              color="amber"
                            />
                          )}
                          ...
                          {row.corner_risk > 0.04 && (
                            <RiskBadge
                              value={row.corner_risk}
                              label="Korner"
                              color="sky"
                            />
                          )}
                          {row.foul_intensity > 0.08 && (
                            <RiskBadge
                              value={row.foul_intensity}
                              label="Faul"
                              color="orange"
                            />
                          )}
                      ...
                      {row.narrative_text && (
                        <p className="text-[11px] text-slate-400 leading-snug pl-12">
                          {row.narrative_text}
                        </p>
                      )}
                    </div>
          ...
          {(!showLiveFlow || showFinished) && (
            <>
              {(showFinished || showLivePending) && prematchRows.length > 0 && (
                <p className="text-[11px] text-navy-500 pt-1 border-t border-navy-800/30">
                  Maç öncesi senaryo analizi:
                </p>
              )}
              {!isLive && !isFinished && (
                <p className="text-[11px] text-navy-500 leading-relaxed">
                  Bu alan gerçek maç sonucu değildir. Next59 modeli; takım
                  geçmişi, eleme performansı, oyuncu profilleri, venue
                  psikolojisi ve kadro güncellemelerine göre 5 dakikalık akış
                  projeksiyonu üretir.
                </p>
              )}
              {!loading &&
                ...
                !isFinished && (
                  <p className="text-xs text-slate-400 py-2">
                    5 dakikalık maç senaryosu hazırlanıyor.
                  </p>
                )}
              {prematchRows.length > 0 && (
                <div className="space-y-0.5">
                  {prematchRows.map((row) => {
                    ...
                    const maxCard = Math.max(
                      row.yellow_card_risk_home,
                      row.yellow_card_risk_away,
                    );
                    const maxCorner = Math.max(
                      row.corner_risk_home,
                      row.corner_risk_away,
                    );
                    const maxFoul = Math.max(
                      row.foul_risk_home,
                      row.foul_risk_away,
                    );
                    return (
          ...
          <div className="flex items-start gap-2 pt-1 border-t border-navy-800/40">
            ...
            <p className="text-xs text-navy-500 leading-relaxed">
              Bu çalışma istatistiksel senaryo analizidir. Kesin sonuç vaadi
              içermez.
            </p>
          </div>
        </div>
  ...
  const statRows: StatRowDef[] =
    hasStats && stats
      ? [
          {
            label: "Toplam Şut",
            h: safeNum(stats.home_total_shots),
            a: safeNum(stats.away_total_shots),
            fmt: (v) => fmtStat(v, 1),
          },
          {
            label: "İsabetli Şut",
            h: safeNum(stats.home_shots_on_target),
            a: safeNum(stats.away_shots_on_target),
            fmt: (v) => fmtStat(v, 1),
          },
          {
            label: "Top Hakimiyeti",
            h: safeNum(stats.home_possession_pct),
            a: safeNum(stats.away_possession_pct),
            fmt: (v) => fmtPct(v),
          },
          {
            label: "Korner",
            h: safeNum(stats.home_corners),
            a: safeNum(stats.away_corners),
            fmt: (v) => fmtStat(v, 1),
          },
          {
            label: "Faul",
            h: safeNum(stats.home_fouls),
            a: safeNum(stats.away_fouls),
            fmt: (v) => fmtStat(v, 1),
          },
          {
            label: "Sarı Kart",
            h: safeNum(stats.home_yellow_cards),
            a: safeNum(stats.away_yellow_cards),
            fmt: (v) => fmtStat(v, 2),
          },
          {
            label: "Kırmızı Kart",
            h: safeNum(stats.home_red_cards),
            a: safeNum(stats.away_red_cards),
            fmt: (v) => fmtStat(v, 2),
          },
          {
            label: "Ofsayt",
            h: safeNum(stats.home_offsides),
            a: safeNum(stats.away_offsides),
            fmt: (v) => fmtStat(v, 1),
          },
          ...(safeNum(stats.home_xg) != null || safeNum(stats.away_xg) != null
            ? [
                {
                  label: "xG",
                  h: safeNum(stats.home_xg),
                  a: safeNum(stats.away_xg),
                  fmt: (v: number | null) => fmtStat(v, 2),
                },
              ]
            : []),
        ]
    ...
    <div className="border border-navy-800/60 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-white">
            Olası Maç İstatistik Projeksiyonu
          </span>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-3 space-y-3 bg-navy-900/20">
          {loading ? (
            <div className="space-y-2">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="h-4 bg-navy-800/30 rounded animate-pulse"
                />
              ))}
            </div>
          ) : !hasStats ? (
            <p className="text-xs text-slate-400 py-2">
              İstatistik projeksiyonu hazırlanıyor.
            </p>
          ) : (
            stats && (
              <>
                ...
                <div className="flex items-center justify-center gap-4 py-2.5 bg-navy-800/40 rounded-lg">
                  ...
                  <span className="text-xl font-black font-mono text-champagne tabular-nums">
                    {fmtStat(safeNum(stats.home_goals_projection), 1)} –{" "}
                    {fmtStat(safeNum(stats.away_goals_projection), 1)}
                  </span>
                  <span className="text-xs font-semibold text-slate-300 truncate max-w-[80px]">
                ...
                {safeNum(stats.confidence) != null && (
                  <div className="flex items-center justify-center">
                    ...
                    <span className="text-[11px] font-semibold text-sky-400 ml-1">
                      {Math.round(safeNum(stats.confidence)! * 100)}%
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-2 pt-1">
                  {statRows.map(({ label, h, a, fmt }) => {
                    ...
                    const total = hn + an;
                    const hPct =
                      total > 0 ? Math.round((hn / total) * 100) : 50;
                    const aPct = 100 - hPct;
                    return (
                      <div key={label}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-mono text-champagne tabular-nums">
                            {fmt(h)}
                          </span>
                        ...
                        <div className="h-1.5 bg-navy-800 rounded-full overflow-hidden flex gap-px">
                          <div
                            className="bg-champagne/70 rounded-l-full transition-all"
                            style={{ width: `${hPct}%` }}
                          />
                          <div
                            className="bg-sky-400/70 rounded-r-full transition-all"
                            style={{ width: `${aPct}%` }}
                          />
                        </div>
                      </div>
          ...
          <div className="flex items-start gap-2 pt-1 border-t border-navy-800/40">
            ...
            <p className="text-xs text-navy-500 leading-relaxed">
              Bu çalışma, maç öncesi mevcut verilerle hazırlanmış yapay zekâ
              destekli istatistiksel senaryo analizidir. Kesin sonuç vaadi
              içermez.
            </p>
          </div>
        </div>
...
function WcOldScenariosSection({
  fixtureUuid,
}: {
  fixtureUuid: string | null;
}) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then((res) => {
        ...
        const vers = [
          ...new Set(
            (res.data as { scenario_version: number }[]).map(
              (r) => r.scenario_version,
            ),
          ),
        ];
        setVersions(vers);
    ...
    <div className="border border-navy-800/40 rounded-xl overflow-hidden opacity-70 hover:opacity-100 transition-opacity">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-navy-800 text-slate-500 font-mono">
            {versions.length} versiyon
          </span>
        </div>
        <ChevronDown
          className={`w-4 h-4 text-navy-600 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        />
      </button>
        ...
        <div className="px-4 pb-4 pt-3 space-y-3 bg-navy-900/10">
          ...
          <div className="space-y-0.5">
            {oldRows.map((row) => (
              <div
                key={row.period_start}
                className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-md bg-navy-800/15 border border-white/[0.02]"
              >
                <span className="shrink-0 text-[11px] font-bold font-mono w-10 text-slate-500">
                  {row.period_label}'
                </span>
...
function RiskBar({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-slate-300 w-28 shrink-0 leading-tight">
        {label}
      </span>
      ...
      <span className="text-xs font-mono tabular-nums text-slate-200 w-9 text-right font-semibold">
        {pct}%
      </span>
    </div>
  );
...
function TeamStrengthRow({
  ...
  profile: WcTeamProfile;
  side: "home" | "away";
}) {
  const isHome = side === "home";
  const elo = Math.round(profile.historical_elo_rating);
  const si = Math.round(
    profile.injury_adjusted_strength_index ??
      profile.wc2026_team_strength_index,
  );
  return (
    <div
      className={`flex flex-col gap-1.5 ${isHome ? "items-start" : "items-end"}`}
    >
      <div
        className={`flex items-center gap-1.5 ${isHome ? "" : "flex-row-reverse"}`}
      >
      ...
      >
        ...
        <span className="text-xs text-slate-300">
          {confidenceTr(profile.calibration_confidence)} güven
        </span>
      </div>
    </div>
  );
...
const CONF_BADGE: Record<string, { label: string; cls: string }> = {
  UEFA: {
    label: "UEFA",
    cls: "bg-blue-900/50 text-blue-300 border-blue-700/40",
  },
  CONMEBOL: {
    label: "CONMEBOL",
  ...
  CAF: {
    label: "CAF",
    cls: "bg-orange-900/50 text-orange-300 border-orange-700/40",
  },
...
function EnrichedQualifierPanel({
  stats,
  side,
}: {
  ...
  const gdColor =
    gd > 0 ? "text-emerald-400" : gd < 0 ? "text-red-400" : "text-slate-400";
  const badge = CONF_BADGE[stats.confederation];

  return (
    <div
      className={`flex flex-col gap-1 ${isHome ? "items-start" : "items-end"}`}
    >
      {badge && (
        <span
          className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${badge.cls} mb-0.5`}
        >
          {badge.label}
        </span>
      )}
      <div
        className={`flex items-center gap-1 flex-wrap ${isHome ? "" : "justify-end"}`}
      >
        <span className="text-xs bg-navy-800 px-1.5 py-0.5 rounded text-slate-200 font-mono">
          {stats.matches_played}O
        </span>
        <span className="text-xs bg-emerald-900/50 px-1.5 py-0.5 rounded text-emerald-300 font-mono">
          {stats.wins}G
        </span>
        <span className="text-xs bg-navy-800 px-1.5 py-0.5 rounded text-slate-300 font-mono">
          {stats.draws}B
        </span>
        <span className="text-xs bg-red-900/40 px-1.5 py-0.5 rounded text-red-300 font-mono">
          {stats.losses}M
        </span>
      </div>
      <div
      ...
      >
        ...
        <span className="text-xs text-champagne font-bold">
          {stats.points} pts
        </span>
      </div>
      <div
      ...
      >
        ...
        <span className="text-xs tabular-nums text-slate-300 font-mono">
          {winPct}% G
        </span>
      </div>
      {stats.avg_total_shots != null && (
        <div
          className={`flex items-center gap-1 text-xs text-slate-400 ${isHome ? "" : "flex-row-reverse"}`}
        >
          <span className="text-slate-500">Şut:</span>
          <span className="font-mono text-slate-300">
            {stats.avg_total_shots.toFixed(1)}
          </span>
          {stats.avg_shots_on_goal != null && (
            <span className="text-slate-500">
              ({stats.avg_shots_on_goal.toFixed(1)} isab.)
            </span>
          )}
        </div>
      )}
      {stats.avg_possession_pct != null && (
        ...
        >
          ...
          <span className="font-mono text-slate-300">
            {stats.avg_possession_pct.toFixed(0)}%
          </span>
        </div>
      )}
      {stats.xg_per_match != null && (
        <div
          className={`flex items-center gap-1 text-xs ${isHome ? "" : "flex-row-reverse"}`}
        >
          <span className="text-slate-500">xG:</span>
          <span className="font-mono text-sky-300 font-semibold">
            {stats.xg_per_match.toFixed(2)}/maç
          </span>
        </div>
      )}
    </div>
    ...
    async function load() {
      const [luRes, pqRes, rfRes] = await Promise.all([
        ...
        supabase
          ...
          .select(
            "wc_referee_profiles(name,country,matches,yellow_cards,direct_red_cards,second_yellow_red_cards,yellow_cards_per_match,direct_red_cards_per_match,total_red_card_effect_per_match,total_cards,total_cards_per_match,card_tendency,red_card_scenario_risk,match_flow_interruption_risk)",
          )
          .eq("fixture_id", fixtureStringId)
          .maybeSingle(),
      ]);
      ...
      const rfData = rfRes.data as {
        wc_referee_profiles: RefereeProfileRow;
      } | null;
      ...
      {(homeStarters.length > 0 || awayStarters.length > 0) && (
        <div className="bg-navy-900/50 border border-navy-800 rounded-xl p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { code: homeTeamCode, starters: homeStarters, side: "Ev Sahibi" },
              { code: awayTeamCode, starters: awayStarters, side: "Deplasman" },
            ].map(({ code, starters, side }) => (
              <div key={code}>
                <p className="text-[10px] font-bold uppercase tracking-wider text-navy-400 mb-2">
                  {side} · {code}
                </p>
                <div className="space-y-1">
                  {starters.map((p) => (
                    <div
                      key={`${p.shirt_number}-${p.player_name}`}
                      className="flex items-center gap-1.5"
                    >
                      <span className="text-[10px] font-mono text-navy-400 w-4 shrink-0 text-right">
                        {p.shirt_number}
                      </span>
                      <span className="text-[10px] font-bold text-navy-500 w-6 shrink-0">
                        {POSITION_TR[p.position] ?? p.position}
                      </span>
      ...
      {qualities.length > 0 && (
        <div className="bg-navy-900/50 border border-navy-800 rounded-xl p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {qualities.map((q) => (
              <div
                key={`${q.team_code}-${q.player_name}`}
                className="bg-navy-800/40 rounded-lg p-3"
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-white truncate">
                    {q.player_name}
                  </span>
                  <span className="text-xs font-mono font-bold text-champagne ml-1 shrink-0">
                    {q.last_match_rating.toFixed(1)}
                  </span>
                </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                {referee.country && (
                  <p className="text-xs text-navy-400">{referee.country}</p>
                )}
              </div>
              ...
              {referee.yellow_cards_per_match != null && (
                <div className="bg-navy-800/50 rounded-lg p-2">
                  <div className="text-xs text-amber-400 font-bold">
                    {referee.yellow_cards_per_match.toFixed(2)}
                  </div>
              ...
              {referee.direct_red_cards_per_match != null && (
                <div className="bg-navy-800/50 rounded-lg p-2">
                  <div className="text-xs text-red-400 font-bold">
                    {referee.direct_red_cards_per_match.toFixed(2)}
                  </div>
                  <div className="text-[10px] text-navy-400 mt-0.5">
                    Kırmızı/Maç
                  </div>
              ...
              {referee.total_cards_per_match != null && (
                <div className="bg-navy-800/50 rounded-lg p-2">
                  <div className="text-xs text-slate-200 font-bold">
                    {referee.total_cards_per_match.toFixed(2)}
                  </div>
                  <div className="text-[10px] text-navy-400 mt-0.5">
                    Top. Kart/Maç
                  </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div>
                <div className="text-[10px] text-navy-500 mb-0.5">
                  Kart Eğilimi
                </div>
                <div
                  className={`font-semibold ${tendencyColor(referee.card_tendency)}`}
                >
              ...
              <div>
                ...
                <div
                  className={`font-semibold ${tendencyColor(referee.red_card_scenario_risk)}`}
                >
              ...
              <div>
                ...
                <div
                  className={`font-semibold ${tendencyColor(referee.match_flow_interruption_risk)}`}
                >
...
function WcPredictionPanel({
      ...
      try {
        ...
        const [
          ...
          supabase
            ...
            .select(
              "provider_team_id,team_name,confederation,matches_played,wins,draws,losses,goals_for,goals_against,goal_difference,points,win_rate,goals_for_per_match,goals_against_per_match,avg_possession_pct,avg_total_shots,avg_shots_on_goal,avg_corners,avg_yellow_cards,total_xg,xg_per_match",
            )
            .eq("provider", "api_football")
            .in("provider_team_id", [
              String(homeApiTeamId),
              String(awayApiTeamId),
            ]),

          supabase
            .from("wc2026_match_90min_scenarios")
            .select(
              "tempo_profile,first_15_story,minutes_15_30_story,minutes_30_45_story,minutes_45_60_story,minutes_60_75_story,minutes_75_90_story,key_match_triggers,confidence_label,created_at",
            )
            .eq("fixture_id", apiFootballFixtureId)
            .order("created_at", { ascending: false })
            .limit(1)
          ...
          enrichedMap.set(String(row.provider_team_id), {
            ...
            avg_possession_pct:
              row.avg_possession_pct != null
                ...
                : null,
            avg_total_shots:
              row.avg_total_shots != null ? Number(row.avg_total_shots) : null,
            avg_shots_on_goal:
        ...
        setState({
          ...
          scenario90: scenario90Result.data as Wc90MinData | null,
          calibratedAt:
  ...
  }, [fixtureUuid, isTBD, apiFootballFixtureId, homeApiTeamId, awayApiTeamId]);
    ...
    return (
      <div className="bg-navy-900/40 border border-navy-800/60 rounded-xl p-5 mt-6">
        ...
        <p className="text-xs text-navy-400">
          Bu maç için model tahmini hazırlanıyor. Turnuva başlamadan önce
          yayınlanacak.
        </p>
      </div>
    );
  ...
  const hp = displayProbs
    ? Math.round(displayProbs.home_pct)
    : Math.round(scenario.home_win_probability * 100);
  const dp = displayProbs
    ? Math.round(displayProbs.draw_pct)
    : Math.round(scenario.draw_probability * 100);
  const ap = displayProbs
    ? Math.round(displayProbs.away_pct)
    : Math.round(scenario.away_win_probability * 100);
  ...
  const fmtDate = calibratedAt
    ? new Date(calibratedAt).toLocaleDateString("tr-TR", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
  ...
  const scenarioPeriods = scenario90
    ? [
      ...
      ].filter((period): period is { label: string; text: string } =>
        Boolean(period.text),
      )
    : [];
    ...
    <div className="bg-navy-900/40 border border-navy-800/60 rounded-xl p-5 mt-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-slate-300">
            {confidenceTr(scenario.calibration_confidence)} Güven
          </span>
          {fmtDate && (
      ...
      <div>
        ...
        <div className="flex items-end justify-between mb-2">
          <div className="text-center">
            <div
              className={`text-lg font-bold tabular-nums ${leading === "home" ? "text-champagne" : "text-white"}`}
            >
              {hp}%
            </div>
            <div className="text-xs text-slate-300 truncate max-w-[80px]">
              {homeTeamName}
            </div>
          </div>
          <div className="text-center">
            <div
              className={`text-base font-bold tabular-nums ${leading === "draw" ? "text-champagne/80" : "text-slate-400"}`}
            >
              {dp}%
            </div>
          ...
          <div className="text-center">
            <div
              className={`text-lg font-bold tabular-nums ${leading === "away" ? "text-sky-400" : "text-white"}`}
            >
              {ap}%
            </div>
            <div className="text-xs text-slate-300 truncate max-w-[80px]">
        ...
        <div className="border border-navy-800/60 rounded-lg p-4">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="text-center px-2">
              ...
              <div
                className={`text-base font-bold tabular-nums ${scenario.strength_diff > 0 ? "text-champagne" : scenario.strength_diff < 0 ? "text-sky-400" : "text-slate-400"}`}
              >
                {scenario.strength_diff > 0 ? "+" : ""}
                {Math.round(scenario.strength_diff)}
              </div>
            </div>
            {awayProfile ? (
      ...
      <div className="space-y-2.5">
        ...
        <RiskBar
          value={scenario.late_goal_probability}
          label="Geç Gol Riski"
          color="amber"
        />
        <RiskBar
          value={scenario.wc2026_chaos_probability}
          label="Kaos Olasılığı"
          color="red"
        />
        <RiskBar
          value={scenario.comeback_probability}
          label="Geri Dönüş İhtimali"
          color="sky"
        />
        <RiskBar
          value={scenario.wc2026_fatigue_risk}
          label="Yorgunluk Riski"
          color="amber"
        />
        <RiskBar
          value={scenario.first_half_goal_probability}
          label="İlk Yarı Gol İhtimali"
          color="emerald"
        />
      </div>
      ...
      <Wc5MinFlowPanel
        ...
        isTBD={isTBD}
        hideFinishedResult={isPageFinished}
      />
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-navy-800/40 rounded-lg p-3">
          ...
          <div className="text-sm font-semibold text-white capitalize">
            {scenario.set_piece_threat
              ? confidenceTr(scenario.set_piece_threat)
              : "—"}
          </div>
        </div>
      </div>
      ...
      <p className="text-xs text-slate-400 leading-relaxed pt-1 border-t border-navy-800/40">
        Bu çalışma, maç öncesi mevcut verilerle hazırlanmış yapay zekâ destekli
        istatistiksel senaryo analizidir. Kesin sonuç vaadi içermez.
      </p>
    </div>
  );
}
...
function FinishedResultBlock({
  ...
  useEffect(() => {
    ...
    supabase
      .from("wc2026_match_scenario_calibration")
      .select(
        "home_win_probability,draw_probability,away_win_probability,predicted_score_home,predicted_score_away",
      )
      .eq("api_football_fixture_id", apiFootballFixtureId)
      .order("calibrated_at", { ascending: false })
      .limit(1)
      ...
      .then(({ data, error }) => {
        ...
        setHomeWinProb(probabilityToPercent(data.home_win_probability));
        setDrawProb(probabilityToPercent(data.draw_probability));
        setAwayWinProb(probabilityToPercent(data.away_win_probability));
        setPredictedHome(
          data.predicted_score_home != null
            ? Number(data.predicted_score_home)
            : null,
        );
        setPredictedAway(
          data.predicted_score_away != null
            ? Number(data.predicted_score_away)
            : null,
        );
      });
  }, [apiFootballFixtureId]);
  ...
  const statusLabel =
    statusShort === "AET" ? "UZS" : statusShort === "PEN" ? "PEN" : "MS";

  const actualOutcome = getActualOutcome(homeScore, awayScore);
  const hasPredictedScore = predictedHome != null && predictedAway != null;
  const exactScoreCorrect = hasPredictedScore
    ? predictedHome === homeScore && predictedAway === awayScore
    : null;

  const hasProbs =
    homeWinProb != null && drawProb != null && awayWinProb != null;

  const probabilityPredictedOutcome = hasProbs
    ? getPredictedOutcomeFromProbabilities(homeWinProb, drawProb, awayWinProb)
    : null;

  const outcomeCorrect =
    probabilityPredictedOutcome != null
      ? probabilityPredictedOutcome === actualOutcome
      : null;
  ...
  const predictedOutcomeProbability =
    probabilityPredictedOutcome === "home"
      ? homeWinProb
      : probabilityPredictedOutcome === "away"
        ? awayWinProb
        : probabilityPredictedOutcome === "draw"
          ? drawProb
          : null;
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <div className="ml-auto flex flex-wrap justify-end gap-1.5">
              {exactScoreCorrect != null && (
                ...
                >
                  {exactScoreCorrect ? "Tam Skor Doğru" : "Tam Skor Yanlış"}
                </span>
              )}
              ...
              {outcomeCorrect != null && (
                ...
                >
                  {outcomeCorrect ? "1X2 Sonucu Doğru" : "1X2 Sonucu Yanlış"}
                </span>
              )}
            </div>
          </div>
              ...
              <div className="bg-navy-800/40 rounded-xl p-3 text-center">
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-black font-mono text-champagne tabular-nums">
                      {homeScore} – {awayScore}
                    </div>
                  </div>
                </div>
                <div
                  className={`mt-2 text-xs font-semibold ${
                    exactScoreCorrect ? "text-emerald-400" : "text-red-400"
                  }`}
                >
                  {exactScoreCorrect
                    ? "Skor birebir doğru tahmin edildi."
                    : "Skor birebir eşleşmedi."}
                </div>
              </div>
            )}
            ...
            {hasProbs && (
              <div className="bg-navy-800/40 rounded-xl p-3">
                <div className="text-xs text-slate-400 mb-2 text-center">
                  1X2 Sonuç Karşılaştırması
                </div>

                <div className="mb-3 grid grid-cols-2 gap-2 text-center">
                  <div className="rounded-lg bg-navy-900/50 p-2">
                    <div className="text-[10px] uppercase tracking-wider text-slate-500">
                      Model Seçimi
                    </div>
                    <div className="mt-1 text-xs font-bold text-champagne">
                      {getOutcomeLabel(
                  ...
                  <div className="rounded-lg bg-navy-900/50 p-2">
                    ...
                    {actualOutcomeProbability != null && (
                      <div className="mt-0.5 text-[10px] font-mono text-slate-400">
                        Model olasılığı %{actualOutcomeProbability}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {probabilityRows.map((row) => {
                    const isModelChoice =
                      row.outcome === probabilityPredictedOutcome;
                    const isActual = row.outcome === actualOutcome;
                ...
                <div
                  className={`mt-2 pt-2 border-t border-navy-700/40 text-xs text-center font-semibold ${
                    outcomeCorrect ? "text-emerald-400" : "text-red-400"
                  }`}
                >
                  {outcomeCorrect
                    ? "En yüksek 1X2 olasılığı gerçekleşen sonuçla eşleşti."
                    : "En yüksek 1X2 olasılığı gerçekleşen sonuçla eşleşmedi."}
                </div>
              </div>
            )}
          </div>
...
export default function WcFixtureDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<"info" | "h2h" | "home" | "away">(
    "info",
  );
  ...
  useEffect(() => {
    if (
      !fixture ||
      fixture.home_team_code === "TBD" ||
      fixture.away_team_code === "TBD"
    )
      return;
    let cancelled = false;
    setDbFixture(null);
    ...
    const query = hasPrimaryKey
      ? supabase
          ...
          .select(
            "id, api_football_fixture_id, home_api_team_id, away_api_team_id, final_home_score, final_away_score",
          )
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .maybeSingle()
      .then(({ data }) => {
        if (!cancelled && data && data.home_score != null)
          setPageMatchState({
  ...
  const isTBD =
    fixture.home_team_code === "TBD" || fixture.away_team_code === "TBD";
  ...
  const isPageFinished =
    pageMatchState !== null &&
    FINISHED_STATUSES.has(pageMatchState.status_short);
  ...
  const isLikelyFinished =
    !isTBD && Date.now() > kickoffMs + LIVE_MATCH_DURATION_MS;
  ...
  return (
    <div className="min-h-screen">
      <SEO
        title={
          isTBD
            ? "WC 2026 Maç Senaryosu — Next59"
            : `${fixture.home_team} - ${fixture.away_team} Maç Senaryosu — Next59`
        }
        description={
          isTBD
            ? "2026 Dünya Kupası maçı için Next59 olasılık ve senaryo analizi."
            : `${fixture.home_team} - ${fixture.away_team} maçı için Next59 olasılık, skor beklentisi ve senaryo analizi.`
        }
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex justify-center mb-6">
            ...
            >
              ...
              {stageLabel}
              {groupLabel ? ` · ${groupLabel}` : ""}
              <span className="text-navy-400 font-mono ml-1 text-xs">
                #{fixture.match_no}
              </span>
            </span>
          </div>
          ...
          <div className="flex items-center justify-center gap-4 sm:gap-10 mb-6">
            ...
            <div className="flex flex-col items-center gap-2.5 flex-1 max-w-[170px]">
              {homeCountry ? (
                <span
                  className={`fi fi-${homeCountry.iso2} rounded-[4px] shadow-md`}
                  style={{ width: 56, height: 38, display: "inline-block" }}
                />
              ) : (
            ...
            <div className="flex flex-col items-center gap-2.5 flex-1 max-w-[170px]">
              {awayCountry ? (
                <span
                  className={`fi fi-${awayCountry.iso2} rounded-[4px] shadow-md`}
                  style={{ width: 56, height: 38, display: "inline-block" }}
                />
              ) : (
                <div className="w-14 h-10 rounded-lg bg-navy-800/60 border border-navy-700 flex items-center justify-center">
      ...
      {isLikelyFinished && !isPageFinished && (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 mt-4 mb-6">
          <div className="bg-navy-900/60 border border-navy-800 rounded-2xl p-4 text-center">
            ...
            <p className="text-sm text-slate-300">
              Sonuç verisi güncelleniyor, kısa süre içinde görüntülenecek.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
            ...
            <div className="space-y-2.5">
                ...
                <div>
                  ...
                  {venue && (
                    <p className="text-xs text-slate-400">
                      {venue.city_display} · {venue.country_tr}
                    </p>
                  )}
                </div>
              </div>
              {venue?.capacity && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-white">
                    {venue.capacity.toLocaleString("tr-TR")}
                  </span>
                </div>
              )}
              <div className="flex items-center justify-between">
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={side}
                className="bg-navy-900/50 border border-navy-800 rounded-xl p-3"
              >
                <p className="text-xs text-navy-400 mb-2">{side}</p>
                <div className="flex items-center gap-2.5">
                  {country ? (
                    <span
                      className={`fi fi-${country.iso2} rounded-[3px] shadow-sm shrink-0`}
        ...
        <div className="bg-navy-900/40 border border-navy-800 rounded-xl p-4">
          {activeTab === "info" && (
            <>
              <p className="text-xs font-bold uppercase tracking-wider text-navy-400 mb-3">
                {isGroupStage
                  ? `Grup ${fixture.group} — Diğer Maçlar`
                  : "Turnuva Bağlamı"}
              </p>
              {isGroupStage ? (
                <GroupContext
                  fixture={fixture}
                  allFixtures={ALL_WC2026_FIXTURES}
                />
              ) : (
                <div className="text-sm text-navy-400 py-4 text-center">
                  <Shield className="w-8 h-8 mx-auto mb-2 text-navy-700" />
                  <p>
                    Bu bir eleme maçı. Gruplar tamamlandıktan sonra rakipler
                    belli olacak.
                  </p>
                </div>
              )}
          ...
          {activeTab === "h2h" && isTBD && (
            <p className="text-sm text-navy-400 py-6 text-center">
              Rakipler henüz belli olmadığından geçmiş karşılaşma
              gösterilemiyor.
            </p>
          )}
          {activeTab === "home" && (
            <>
              <p className="text-xs font-bold uppercase tracking-wider text-navy-400 mb-3">
                {homeCountry?.name_tr ?? fixture.home_team} — Dünya Kupası
                Geçmişi (1930–2022)
              </p>
              {homeCountry ? (
                <TeamPastWC
                  teamCode={fixture.home_team_code}
                  teamNameEn={homeCountry.name_en}
                />
              ) : (
                <p className="text-sm text-navy-400 py-6 text-center">
                  Takım henüz belli olmadı.
                </p>
              )}
            </>
          )}
          {activeTab === "away" && (
            <>
              <p className="text-xs font-bold uppercase tracking-wider text-navy-400 mb-3">
                {awayCountry?.name_tr ?? fixture.away_team} — Dünya Kupası
                Geçmişi (1930–2022)
              </p>
              {awayCountry ? (
                <TeamPastWC
                  teamCode={fixture.away_team_code}
                  teamNameEn={awayCountry.name_en}
                />
              ) : (
                <p className="text-sm text-navy-400 py-6 text-center">
                  Takım henüz belli olmadı.
